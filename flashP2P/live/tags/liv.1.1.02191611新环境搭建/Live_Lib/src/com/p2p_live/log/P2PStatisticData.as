package com.p2p_live.log
{
	...
	public class P2PStatisticData extends EventDispatcher
	{
		...
		private var _httpsizeNum:uint=0;//http片大小
		...
		public function setInitTime(netstream:P2PNetStream):void
		{
			if(!_netstream)
			{
				_netstream = netstream;			
		...
		private function netstatusHandle(event:Object):void
		{
			...
			switch (code)
			{
				...
				case "P2P.P2PGetChunk.Success":	
					/*trace("event.info.begin  "+event.info.begin)
					trace("event.info.end  "+event.info.end)*/
		...
		public function getStatisticData():Object
		{
			...
			obj.type     = "live";
			obj.p2ptime  = Math.round(p2ptimeNum);     //心跳周期内p2p下载耗费时间 (毫秒)     Number
			obj.p2psize  = Math.round(_p2psizeNum);    //心跳周期内下载p2p大小（字节）		Number			
			obj.chunknum = _numChunks;                 //总chunk数					        uint
			obj.alltime  = Math.round(alltimeNum*1000);//心跳周期内总下载耗费时间(毫秒)	    Number
		...
		private function kernelReport(obj:Object):void
		{
			var object:Object = new Object();
			object.csize = obj.httpsize;
			object.dsize = obj.p2psize;
