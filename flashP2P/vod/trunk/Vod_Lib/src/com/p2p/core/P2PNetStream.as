		...
		*/
		...
		public function set_CDN_URL(arr:Array):void
		{
				...
				&& _videoInfoObj.hasOwnProperty("flvURL"))
			{
				...
				_videoInfoObj["flvURL"] =  ArrayClone.Clone(arr);
		...
		public function callBackFunction(obj:Object):void
		{
		...
		override public function play(...arguments):void
		{
			...
			*/
			...
			{
				_videoInfoObj["groupName"] = getSHA1Code(_videoInfoObj.groupName+VodConfig.P2P_AGREEMENT_VERSION);/*VodConfig.P2P_AGREEMENT_VERSION + _videoInfoObj["groupName"]*/;
			}
		...
		{
			//trace(this,"_time"+_time,_isSeeking,super.time);
		...
		protected function reset():void
		{
			...
			tmpLastBufferLength = 0;
			tmpBufferLengthCount = 0;
			_playBuffering = false;
			_isForcedAddCheck = false;
		...
		protected function _timer_TIMER(event:TimerEvent):void
		{
			...
			//trace("time "+time+" ,bufferlength "+this.bufferLength)
			if (_isSeeking)
			{
//				_countBuffer++;
...
//			_countBuffer=0;//不是seek该值恢复为0
			...
			if ( (this.bufferLength < this.bufferTime + 1) || _isForcedAddCheck )
			{				
				_isForcedAddCheck = false;				
				tmpLastBufferLength = 0;
				tmpBufferLengthCount = 0;
			...
			}else if (!_isPause)
			{
				...
				if (_metaDataReady)
				{
					if(tmpLastBufferLength == this.bufferLength)
					{
						tmpBufferLengthCount++;
						if( tmpBufferLengthCount >= 3 )
						{
							var bytes1:ByteArray = _manager.readByteArray(_byteArrayIndex)
							if( bytes1 != null )
							{
								_isForcedAddCheck = true;
							}else
							{
								seekToNextKeyFrame(this.time);								
							}
					...
					tmpLastBufferLength = this.bufferLength;
				}
			}
		...
		private var tmpLastBufferLength:Number = 0;
		private var tmpBufferLengthCount:uint = 0;
		private var _playBuffering:Boolean = false;
		private var _isForcedAddCheck:Boolean = false;
