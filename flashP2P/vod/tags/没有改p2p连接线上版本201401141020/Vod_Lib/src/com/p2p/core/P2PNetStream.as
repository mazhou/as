		...
		*/
		...
		override public function play(...arguments):void
		{
			...
			*/
			...
			for(var m:String in arguments[0])
			{
				_videoInfoObj[m] = arguments[0][m];
				trace(this,m+" == "+_videoInfoObj[m]);
			}
			//_videoInfoObj["flvURL"] = arguments[0]["flvURL"];
			...
			{
				trace(this,"v+groupName = "+VodConfig.P2P_AGREEMENT_VERSION+_videoInfoObj.groupName);
			}
			
			trace(this,"v+flv0 = "+VodConfig.P2P_AGREEMENT_VERSION+parseUrl(_videoInfoObj["flvURL"][0]));
			//_videoInfoObj.groupName = sha1Encrypt.encrypt(VodConfig.P2P_AGREEMENT_VERSION+_videoInfoObj.groupName);
			var sha1:sha1Encrypt = new sha1Encrypt(false);
			
			_videoInfoObj.groupName = sha1Encrypt.encrypt(VodConfig.P2P_AGREEMENT_VERSION+parseUrl(_videoInfoObj["flvURL"][0]));
			trace(this,"_videoInfoObj.groupName = "+_videoInfoObj.groupName);
		...
		{						
			...
			switch (code)
			{
				...
				case "P2P.LoadCheckInfo.Success" :
					...
					if(_videoInfoObj.hasOwnProperty("canCheck"))
					{
						object.info = VodConfig.VERSION+" canCheck= "+_videoInfoObj.canCheck;
					}
					else
					{
						object.info = VodConfig.VERSION+" canCheck= true";
					}
					callBackFunction(object);
					//
