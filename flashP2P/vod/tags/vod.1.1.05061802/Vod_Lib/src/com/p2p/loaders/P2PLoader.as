...
//			_gatherServiceClear();
		...
		{
			_natType = _STUNConnecter.natType;
			_STUNConnecter.removeEventListener(Event.COMPLETE,STUNConnecterSuccess);
			_STUNConnecter.clear();
			_STUNConnecter = null;
		}/**/
		private function gatherRegisterTimer(event:* = null):void
		{
				...
				&& _pipeNum < _MaxConnectedPeers)
			{
				...
				try
				{
					...
					if( _natType != -1 )
					{
						_url = String(_url+"&nat="+_natType);
						if(P2PCookie.GetNatType() != _natType)
						{
							P2PCookie.SetNatType(_natType);
						}
					}
					else
					{
