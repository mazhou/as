package com.p2p.core
{
	...
	public class P2PNetStream extends NetStream
	{
		...
		protected var _version:String = "vod.1.1.05241412";
		...
		*/
		...
		public function P2PNetStream()
		{
			...
			//----------------------------设置内部监控上报
			KernelReport.SET_INFO(_version,"vod");	        
			//----------------------------
		}
		...
		public function set outMsg(fun:Function):void
		{
			...
			{
				_outMsg = fun;
				_outMsg.call(null,_version,"version");
			}
			else
			{
				_outMsg = null;
		...
		protected function metaDataLoaderStatus(e:MetaDataLoaderEvent):void
		{
			...
			if(e.info.code == MetaDataLoaderEvent.LOAD_METADATA_SUCCESS)
			{
				...
				if(_bytesTotal == 0) 
				{
					...
					//
					_videoInfoObj.urgenceBufferSize = (_bytesTotal / _duration)*60 / _chunkSize;
					_videoInfoObj.kbps = obj.metaData.videodatarate;
					/**
					 * 判断是否支持在播放广告时开启P2P加载
					 * 当存在广告剩余时间变量并且广告剩余时间大于 5秒 时才开启P2P优先加载
					 * */
					if( _videoInfoObj.adTime                        //当存在广告剩余时间变量
						&& (_videoInfoObj.adTime*1000-obj.utime)>5*1000  //当广告剩余时间大于5秒
						)
					{
						_videoInfoObj.adTime = _videoInfoObj.adTime*1000-obj.utime;
					}
					else
					{
						_videoInfoObj.adTime = 0;
					}
					/**************************/
