package com.p2p.managers
{
	...
	public class DataManager extends EventDispatcher
	{
		...
		protected static const MEMORY_LENGTH:uint = 300*1024*1024;
		...
		{ 			
			...
			trace(_kbps);
			//trace(_arrayFLVURL)
			/*//_arrayFLVURL[0] = "http://123.125.89.72/2/45/16/2021036333.0.letv?crypt=6e1262faaa7f2e200&b=820&qos=4&level=20&nc=3&bf=20&video_type=flv&check=1&tm=1344956400&key=b50c042692cc133e7b61ec8e934f8c80&proxy=1006801958&host=www_letv_com&tag=letv&sign=free"
			_arrayFLVURL[0] = "http://123.125.89.38/2/45/16/2021036333.0.letv?crypt=7c4c4213aa7f2e166&b=820&qos=4&level=20&nc=3&bf=16&video_type=flv&check=1&tm=1344956400&key=b50c042692cc133e7b61ec8e934f8c80&proxy=2071812424&host=www_letv_com&tag=letv&sign=free"
			//_arrayFLVURL[0] = "http://111.161.65.33/2/45/16/2021036333.0.letv?crypt=42b16c26aa7f2e172&b=820&qos=4&level=20&nc=3&bf=17&video_type=flv&check=1&tm=1344956400&key=b50c042692cc133e7b61ec8e934f8c80&proxy=2007487153&host=www_letv_com&tag=letv&sign=free"
			_arrayFLVURL[1] = _arrayFLVURL[0]; 
			_arrayFLVURL[2] = _arrayFLVURL[0]; */
				
				//
			chunks     = new Chunks(MEMORY_LENGTH,clip_interval);
			_p2pLoader = new P2PLoader(this,_videoInfo.geo);
		...
		*/
		...
		{			
			...
			if(obj.from=="http")
			{
				...
				obj.cnod=getCnode(_arrayFLVURL[_arrayFLVURLIndex]);
				//
			}else
			{
			...
			*/
			...
			for(var i:int =0; i < httpArray.length; i++)
			{
				...
				{					
					...
					{
						if(_vodDataLoader.httpLoadData(Uindex, n, _arrayFLVURL[_arrayFLVURLIndex]))
						{
		...
		protected function p2pDispatch():void
		{
			...
			{			
				...
				{							
					...
					if (isHttpZoneFull(_urgenceBufferSize/2) == false)
					{
						remarkWant(0, fileTotalChunks-1);
						return ;
					}else 
		...
		*/
		...
		protected function errorHandler(e:P2PEvent):void
		{
			...
			switch(e.info.type)
			{
				...
				case "HttpLoader-error" :
					...
					obj.url        = _arrayFLVURL[_arrayFLVURLIndex];
					obj.nodeIdx    = _arrayFLVURLIndex;					
					obj.utime      = _utime;
					//
					_arrayFLVURLIndex++;
					//retry从1开始计数
					obj.retry = _arrayFLVURLIndex;
					
					if(_arrayFLVURL[_arrayFLVURLIndex] != undefined)
					{
		...
		{
			//flash.system.Security.loadPolicyFile(_arrayXMLSocketURL[_arrayFLVURLIndex]);
			flash.system.Security.loadPolicyFile(_arrayXMLSocketURL[_arrayFLVURLIndex]);
		}*/
	}
}