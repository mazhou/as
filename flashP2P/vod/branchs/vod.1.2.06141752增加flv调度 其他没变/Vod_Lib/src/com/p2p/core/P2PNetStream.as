package com.p2p.core
{
	...
	import com.p2p.log.ConsoleDebug;
		...
		*/
		...
		public function set_CDN_URL(arr:Array):void
		{
				...
				&& _videoInfoObj.hasOwnProperty("flvURL"))
			{
				...
				trace(this,"change.arr = "+arr);
				trace(this,"change.arrayFLVURL = "+_manager.arrayFLVURL);
		...
		public function set outMsg(fun:Function):void
		{
			...
			{
				_outMsg = fun;
				_outMsg.call(null,VodConfig.VERSION+VodConfig.TEMP_VERSION,"version");
			}
			else
			{
				_outMsg = null;
		...
		/**由播放器调用，传入广告倒计时的时间，当播放器调用此方法时下载Checksum、开始P2P下载数据,单位：秒*/
		public function setAdRemainingTime( adTime:Number ):void
		{
			VodConfig.setAdRemainingTime(adTime*1000);
		}
		...
		override public function play(...arguments):void
		{
			...
			*/
			...
			if(_videoInfoObj["adRemainingTime"] >= 0)
			{
				VodConfig.setAdRemainingTime(_videoInfoObj["adRemainingTime"]*1000);
			}
			
			if(_videoInfoObj["groupName"])
			{
				_videoInfoObj["groupName"] = VodConfig.P2P_AGREEMENT_VERSION + _videoInfoObj["groupName"];
			}
			for(var i:String in _videoInfoObj)
			{
				trace(this,"--内核-- "+i+"   "+_videoInfoObj[i]);
			}				
			/************************************************/
		...
		override public function resume():void
		{
			...
			/**lz 0524 add*/
			//_manager.adTime = 0;
			VodConfig.setAdRemainingTime(0);
			/**************/
			super.resume();
		...
		{
			ConsoleDebug.log(this,"close()");
			reset();
			super.close();
			ConsoleDebug.log(this,"close end");
		}
		/**
		...
		protected function metaDataLoaderStatus(e:MetaDataLoaderEvent):void
		{
			...
			if(e.info.code == MetaDataLoaderEvent.LOAD_METADATA_SUCCESS)
			{
				...
				/*if( _duration > 0 && _duration <=  this.bufferTime  )
				{
					_oldBufferTime = this.bufferTime = 0.1;	
				}*/
		...
		{
			ConsoleDebug.log(this,"reset()");
			if(_timer)
			{
				_timer.stop();
				ConsoleDebug.log(this,"_timer.stop()");
			}
		...
		private function setBufferTime( isBufferEmptyDispatch:Boolean=false ):void
		{
			...
			if( _duration >= 15 )
			{
				if( /*getTime()*/this.time-_startPlayTime>=15/*15*1000*/ )
				{
					this.bufferTime = 15;
				}
				else
			...
			if( _duration-this.time < 3 )
			{
				this.bufferTime = 0.1;
			}			
		}
		//---------------------------------------
		private var vipTemp:Boolean = true;
		protected function _timer_TIMER(event:TimerEvent):void
		{
			if( this.time >= _duration )
			{
				//针对http://yuntv.letv.com/bcloud.html?uu=970effdcdb&vu=ffd35d84e9视频无法触发play.stop事件做的处理
...
//				}
				...
				{				
					...
					if (seekBytes)
					{
						...
						_startPlayTime = this.time;
						setBufferTime();
						//trace("time 1 = "+this.time)
						this["appendBytesAction"]("resetSeek");
						this["appendBytes"](seekBytes);
						//trace("time 2 = "+this.time)
...
//			_countBuffer=0;//不是seek该值恢复为0
			...
			{				
				...
				{	
					if( vipTemp )
					{
						//针对http://www.vip.com/detail-182449-24653055.html视频无法正常播放并且播放无法正常结束的情况做的补丁
						seek(_seekToTimeOnStart)
						vipTemp = false;
						return;
					}
					//
...
//						}
					...
					ConsoleDebug.log(this,"NetStream.Buffer.Empty");
					setBufferTime(true);
					break;
...
//					}
		...
		protected function dispatchPlayStopEvent():void
		{
			...
			dispatchEvent(new P2PNetStreamEvent(P2PNetStreamEvent.STREAM_STATUS,info));
			ConsoleDebug.log(this,"dispatch  Stream.Play.Stop");
			//-----------------------lizhuo 0601 14:06 add
			if(_outMsg != null)
			{
				_outMsg.call(null,info.code);
		...
		protected function dispatchPlayFailedEvent(obj:Object,type:String=null):void
		{
			...
			{				
				...
				trace(this,"Stream.Play.Failed, sockStatus == Failed, info.allCDNFailed = "+info.allCDNFailed+", info.allCDNFailed = "+info.allCDNFailed);
				if(_outMsg != null)
				{
			...
			{
				dispatchEvent(new P2PNetStreamEvent(P2PNetStreamEvent.STREAM_STATUS,info));
				trace(this,"Stream.Play.Failed, allCDNFailed = "+info.allCDNFailed);
			}else
			{
				info.p2pErrorCode = "0000";
				dispatchEvent(new P2PNetStreamEvent(P2PNetStreamEvent.P2P_All_OVER,info));
				trace(this,"Stream.Play.Failed, allCDNFailed = "+info.allCDNFailed+", info.allCDNFailed = "+info.allCDNFailed);
			}			
