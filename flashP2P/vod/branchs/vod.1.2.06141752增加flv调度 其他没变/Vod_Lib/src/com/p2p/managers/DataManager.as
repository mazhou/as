package com.p2p.managers
{
	...
	public class DataManager extends EventDispatcher
	{
		...
		public var arrayFLVURL:Array;        //保存flv地址的数组
		public var arrayFLVURLIndex:int = 0 ;//当前使用的flv地址索引
		//protected var _arrayXMLSocketURL:Array;  //保存允许访问的socket端口地址的数组，与arrayFLVURL的索引保持一致
		...
		/*public function setAdTime():void
		{
			if(_adTime > 5*1000)
			{
				_adTime -= getTime()-_locationADTime;
			}			
		}*/
		/** lz0523 add*/
		private function ifP2PFirst():Boolean
		{
			 ...
			 * */
			if(VodConfig.getAdRemainingTime() == 0)
			{
				return false;
			}
			return true;
		...
		*/
		...
		{			
			...
			/**当暂停p2p功能时***************/
			if( _pauseP2P 
				|| false == VodConfig.ifHasSendAdTime() )
			{
				if(isHttpZoneFull(_urgenceBufferSize) == false)
				{
					//如果紧急区不满则进行上报
		...
		*/
		...
		{			
			...
			if(obj.from=="http")
			{
				...
				obj.cnod=getCnode(arrayFLVURL[arrayFLVURLIndex]);
		...
		*/
		protected function httpDispatch(index:uint):void
		{
			...
			if( false == VodConfig.ifHasSendAdTime() )
			{
				return;
			}
			...
			for(var i:int =0; i < httpArray.length; i++)
			{
				...
				{					
					...
					{
						//trace(this,arrayFLVURLIndex+" "+arrayFLVURL[arrayFLVURLIndex])
						if(_vodDataLoader.httpLoadData(Uindex, n, arrayFLVURL[arrayFLVURLIndex]))
						{
		...
		protected function errorHandler(e:P2PEvent):void
		{
			...
			switch(e.info.type)
			{
				...
				case "HttpLoader-error" :
					...
					obj.url        = arrayFLVURL[arrayFLVURLIndex];
					...
					if(arrayFLVURL[arrayFLVURLIndex] != undefined)
					{
						obj.allCDNFailed  = 0;
