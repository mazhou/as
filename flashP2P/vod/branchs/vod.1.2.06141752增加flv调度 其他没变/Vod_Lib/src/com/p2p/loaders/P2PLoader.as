...
//			_gatherServiceClear();
		...
		{			
			...
			{
				if( nowTime-_badPipeList[i]>=30*1000 )
				{
			...
			for(pipeID in _pipeList)
			{
				...
				else if (_pipeList[pipeID])
				{
					...
					{						
						...
						_badPipeList[pipeID] = Math.floor((new Date()).time);
...
//					_gatherService();
				...
				case "NetStream.Connect.Success":
					...
					{
						_pipeList[e.info.stream.farID]._canSend = true;
				...
				if (_publishedOK == false)
				{
					...
					if(_sendNetStream == null )
					{
						...
						{							
							...
							{
								if( _pipeList[callerns.farID]._canSend == true )
								{
									_pipeList[callerns.farID].ExtraNetStream = callerns;
									_pipeList[callerns.farID].ExtraNetStream.client = _pipeList[callerns.farID];
									return true;
								}
								_pipeList[callerns.farID].clear();
								_pipeList[callerns.farID] = null;
								delete _pipeList[callerns.farID];
							...
							{
								if(!_sparePipeList[callerns.farID])
								{
									_sparePipeList[callerns.farID] = callerns.farID;
								}
								return false;
							}*/
