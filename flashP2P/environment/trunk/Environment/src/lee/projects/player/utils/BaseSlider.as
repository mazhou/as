package lee.projects.player.utils{
	...
	public class BaseSlider extends EventDispatcher {
		...
		public function BaseSlider(skin:Sprite,offset:Number=NaN){
			...
			progressSprite.alpha=0.5;
			fullnessSprite.alpha=0.5;
			interactSprite.alpha=0.5;
		...
		public function updateBuffer(num:Number=0):void
		{return;
			if(  GlobalReference.type == "LIVE"  )
			{
				return;
			}
			if(    GlobalReference.type != "HLS" 
				&& RectManager.dataManager
				&& RectManager.dataManager.bufferTimeArray)
			{	
			...
			{
				bufferSprite.graphics.clear();
//				bufferSprite.graphics.drawRect(
				bufferSprite.graphics.beginFill(0x666666,0.5);
				bufferSprite.graphics.drawRect(0,0,width*progress,progressSprite.height);
				bufferSprite.graphics.endFill();
