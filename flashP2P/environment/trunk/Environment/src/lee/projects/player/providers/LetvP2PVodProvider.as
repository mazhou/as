package lee.projects.player.providers{
	...
	public class LetvP2PVodProvider extends EventDispatcher implements IProvider{
		...
		public function LetvP2PVodProvider()
		{
			super();
		...
		private function _play(obj:Object):void
		{
			...
			postRun = (new Date).time;
			_ns.play(obj);
			//_ns.setAdRemainingTime(_adTime);
			//_ns.pause();
			//
		...
		protected function _timer_TIMER(event:TimerEvent):void{
			...
			{
				_ns.set_CDN_URL([flvArr[flvIdx]]);
				flvIdx++;
				if(flvIdx == flvArr.length)
				{
					flvIdx = 0;
				}
				timeCount = 0;
			...
			timeCount += _timer.delay;
		}
		private var flvIdx:int = 0;
		private var timeCount:int = 0;
		private var flvArr:Array=["http://123.125.89.31/24/47/53/letv-uts/1725544-AVC-537997-AAC-31586-2699960-198739996-9efc8d87b10aeade1253c38ede74fc47-1359401984422.letv?crypt=51aa7f2e131&b=588&nlh=3072&nlt=5&bf=18&gn=706&p2p=1&video_type=flv&opck=1&check=0&tm=1379736000&key=936cb642c43653a53eb847c3b3a03f0d&proxy=2071812446&cips=10.58.100.153&geo=CN-1-0-2&lgn=letv&mmsid=2216654&platid=1&splatid=101&playid=0&tss=no&termid=1&hwtype=un&pay=0&tag=letv&sign=letv&rateid=1000",
									"http://60.217.237.134/24/47/53/letv-uts/1725544-AVC-537997-AAC-31586-2699960-198739996-9efc8d87b10aeade1253c38ede74fc47-1359401984422.letv?crypt=80aa7f2e218&b=588&nlh=3072&nlt=5&bf=30&gn=750&p2p=1&video_type=flv&opck=1&check=0&tm=1379736000&key=936cb642c43653a53eb847c3b3a03f0d&proxy=2007470864&cips=10.58.100.153&geo=CN-1-0-2&lgn=letv&mmsid=2216654&platid=1&splatid=101&playid=0&tss=no&termid=1&hwtype=un&pay=0&tag=letv&sign=letv&rateid=1000",
									"http://119.167.147.16/24/47/53/letv-uts/1725544-AVC-537997-AAC-31586-2699960-198739996-9efc8d87b10aeade1253c38ede74fc47-1359401984422.letv?crypt=37aa7f2e220&b=588&nlh=3072&nlt=5&bf=30&gn=730&p2p=1&video_type=flv&opck=1&check=0&tm=1379736000&key=936cb642c43653a53eb847c3b3a03f0d&proxy=2007470958&cips=10.58.100.153&geo=CN-1-0-2&lgn=letv&mmsid=2216654&platid=1&splatid=101&playid=0&tss=no&termid=1&hwtype=un&pay=0&tag=letv&sign=letv&rateid=1000"];
		private var ifHasSendAdTime:Boolean = false;
		private var postRun:Number = 0;
		private var nowRun:Number = 0;
		protected function adTimeOver():void
		{
			nowRun = (new Date).time;
			if(_adTime>0)
			{
				_adTime = _adTime - ( nowRun-postRun)/1000;
				postRun = nowRun;
				var obj:Object = new Object();
				obj.name = "adTime";
				...
				if( _adTime <= 20 && false == ifHasSendAdTime )
				{
					_ns.setAdRemainingTime(20);
					ifHasSendAdTime = true;
				}*/
			}
			/*else
			{
