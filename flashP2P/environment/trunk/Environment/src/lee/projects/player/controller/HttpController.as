...
//	import RectLog.*;
	...
	public class HttpController extends EventDispatcher{
		...
		private var if_m3u8:Boolean = true;//false;//
...
//			videoObject.gslb = "http://live.gslb.letv.com/gslb?stream_id=cctv1&tag=live&ext=xml&format=1&expect=2";
			if(str=="")
			{
				if(if_m3u8)
				{
					videoObject.gslb = "http://live.gslb.letv.com/gslb?stream_id=cctv5_800&tag=live&ext=m3u8&format=1&expect=2";
				}
				else
				{
			...
			}else
			{
				if(if_m3u8)
				{
					videoObject.gslb = "http://live.gslb.letv.com/gslb?stream_id="+str+"&tag=live&ext=m3u8&format=1&expect=2";
				}
				else
				{
