...
//	import lee.managers.RectManager;
	...
	public class HLSController extends EventDispatcher{
		...
		protected var _rectLog:HLSRectLog;
		...
		public function initialize():void{
			...
			_controlbar.addEventListener(PlayerEvent.STATE_CHANGE,player_stop);
			...
			try
			{
			ExternalInterface.addCallback("vid_Debug",vid_Debug);
			}
			catch(err:Error)
			{
				trace("not surpport externalInterface!");
			}
		}
		public function vid_Debug(str:String):void
		{
			loadVInfo(str);
		}
...
//			}
		...
		protected function initRectLog():void
		{
			if(_rectLog)
			{
				...
				GlobalReference.root.removeChild(_p2pInfoArea);
			}
			else
			{}
				_rectLog=new HLSRectLog();
				_rectLog.x=2;
				_rectLog.y=_h-200;
		...
		protected function player_stop(event:PlayerEvent):void{
			initRectLog();
		}
	}
}
