package com.p2p.dataManager
{
	...
	public class LiveDataManager implements IDataManager
	{
		...
		{
			PlayingBlockID = id;
			_blockList.clearDestroy(PlayingBlockID);
		}
...
//		}
		//
		public var httpDownLoadPos:Number = 0;		
		public var earliestStartTimeAllPeer:Number = 0;
		private var lastTask:Block=null;
		public function getDataTask():Block
		{
			if(lastTask)
			{
				if(    (!lastTask.isChecked)
					&& (!lastTask.isDestory)
					&& lastTask._downLoadStat != 1)
				{
					return lastTask; 
				}
				var nextSeqBlock:Block = _blockList.getNextSeqID(lastTask.sequence);
				if(nextSeqBlock 
					&& (!nextSeqBlock.isChecked) 
					&& nextSeqBlock._downLoadStat != 1 )
				{
					if(!nextSeqBlock.isDestory)
					{
						if(nextSeqBlock.id - LIVE_TIME.GetBaseTime() < LiveVodConfig.DAT_BUFFER_TIME)
						{
							lastTask=nextSeqBlock;
							return nextSeqBlock;
						}
					}
			...
			//MaxHttpPos = _p2pLoad.getMaxHttpLoadPos();
			earliestStartTimeAllPeer = _p2pLoad.getAllPeerMaxHttpLoadPos();
				...
				for each(var i:Number in arr)
				{
					if (i >= PlayingBlockID)
					{
						...
						if (block)
						{
							...
							{
								/**在紧急区之内的block，目前为播放点之后的30秒，
								 如果没有下载到数据或下载的数据有问题，则强行将该任务分配给
								 http下载*/
								if (block._downLoadStat != 1 
									&& block.isChecked == false 
									&& block.isDestory == false)
								{
									block._downLoadStat = 1;
									//this.doAddHave();
							...
							if (   /*earliestStartTimeAllPeer == Number.MAX_VALUE||*/
								  earliestStartTimeAllPeer < this.startTime) //如果其他节点的运行时间早于本地运行时间
								return null;
