...
//				}
			...
			_initData.groupName = getSHA1Code(String(_initData.groupName+LiveVodConfig.P2P_AGREEMENT_VERSION));
			//_initData.groupName = "777";
			reset();
			/**开始加载的位置*/
...
//			P2PDebug.traceMsg(this,"r_BaseTime"+LIVE_TIME.GetBaseTime());
		...
		protected function headHandler():void
		{
			...
			if(head 
				&& head.getHeadStream().bytesAvailable >0
				&& (tmpBlock.isChecked || tmpBlock.isDestory)
			)
			{	
				block = tmpBlock;				
		...
		protected function frameHandler():void
		{
			...
			if(block)
			{
				if ((LIVE_TIME.GetBaseTime() > block.id + block.duration/1000 + 3*60) && isReadResetBegin())
				{
...
//			}
			...
			if(!tempblock && LIVE_TIME.GetBaseTime()>block.id)
			{
				var blkID:Number = _dispather.getBlockId(LIVE_TIME.GetBaseTime());
				tempblock = _dispather.getBlock(blkID);
				if(!tempblock){return}
//				if(tempblock.sequence != block.sequence+1){
...
//			}else return;
			//
			if(tempblock && _lastHead!=tempblock.head)
			{
			...
			//
			if(tempblock && tempblock.isDestory && tempblock.sequence == block.sequence+1)
			{
				block=tempblock;
			...
			}/*else if(tempblock && tempblock.isChecked) 
			{
				reseek(tempblock.id,"changeHead !");
			}*/
		}
