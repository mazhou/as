package com.p2p.data
{
	...
	{	
		...
		private var _isDestory:Boolean = false;
		...
		public function set isDestory(b:Boolean):void
		{
			_isDestory = b;
			...
			if(_isDestory)
			{
				idx = _blockList.destoryBlockArr.indexOf(this.id)
				if( idx == -1)
				{
					_blockList.destoryBlockArr.push(this.id);
				}				
			}
		...
		{			
			...
			{
				errorOutMsg("该piece重复添加！",pieceID);
				return false;
			}
			//
			...
			_blockList.streamSize += pieces[pieceID].stream.bytesAvailable;
		...
		public function reset():void
		{
			...
			for(var i:int=0 ; i<pieces.length ; i++)
			{
				if(pieces[i])
				{
					...
					_blockList.streamSize -= pieces[i].stream.bytesAvailable;
