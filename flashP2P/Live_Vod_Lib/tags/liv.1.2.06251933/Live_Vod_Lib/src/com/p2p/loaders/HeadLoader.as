...
//			}
			...
			if(evt){
				P2PDebug.traceMsg(this,"head错误类型"+evt.text);
		...
		override protected function completeHandler(evt:Event):void
		{
			if(_tempArray.length>0){
				P2PDebug.traceMsg(this,"加载头成功"+_name+"继续加载下一个头");
				this.start(_tempArray.shift());
			}else{
				P2PDebug.traceMsg(this,"加载头成功"+_name);
				isDownLoad=false;
			}
		}
...
//			Debug.traceMsg(this,"头调度器getDatURL"+_initData.flvURL[loadURLIndex].replace("desc.xml",_name));
		...
		override protected function jsInterface():void{
		}
	}
}
//		_xml=<root>
