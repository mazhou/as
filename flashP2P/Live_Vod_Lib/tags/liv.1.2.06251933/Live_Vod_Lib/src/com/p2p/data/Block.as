		...
		public function setBlockStream(byteArray:ByteArray):void
		{
			...
			if(doCheckSum(byteArray))
			{
				...
				for(var i:uint=0;i<pieces.length;i++)
				{
					...
					{
						//delete pieces[i].stream;
						pieces[i].stream = null;
					}
					//
					if(i==pieces.length-1)
					{
						readLen=_size-LiveVodConfig.CLIP_INTERVAL*(pieces.length-1);
					}else
					{
						readLen=LiveVodConfig.CLIP_INTERVAL;
					}
					//
					pieces[i].stream=new ByteArray;
					byteArray.position=i*LiveVodConfig.CLIP_INTERVAL;
...
*/			var byteArray:ByteArray=new ByteArray;
		...
		public function setPieceStream(pieceID:uint,byteArray:ByteArray,remoteName:String=""):Boolean
		{
			...
			{
				P2PDebug.traceMsg(this,"已经有数据 "+id+"_"+pieceID+"  ; "+"length = "+pieces[pieceID].stream.length+"  ; iType = "+pieces[pieceID].iLoadType);
				return false;
			}*/
			...
			if(_isFull && !_isChecked)
			{
				...
				else
				{
					this.reset();
					P2PDebug.traceMsg(this,"checkSum有问题！！ "+this.id);
//					ExternalInterface.call("trace","checkSum有问题！！ "+this.id);
					this._isDestroy=true;
					Statistic.getInstance().P2PCheckSumFailed(String(this.id));
				}
				P2PDebug.traceMsg(this,_blockList.count);
			}
...
//				ExternalInterface.call("trace","size有问题！！ "+this.id+" s="+this.size+" l="+byteArray.length);
			}
			return false;
