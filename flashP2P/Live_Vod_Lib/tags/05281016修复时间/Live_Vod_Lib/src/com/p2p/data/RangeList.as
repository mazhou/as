...
//						_rangeList[block.nextID+"_0"].startBlockID+"_0");
		...
		private function eliminate():void
		{
			...
			{
				Debug.traceMsg(this,"_count = "+_count);
				Debug.traceMsg(this,"_playHead = "+_playHead+" "+TimeTranslater.getTime(_playHead));
				...
				{//播放点所在的小时小于最早流所在的小时
					Debug.traceMsg(this,"eliminateForback0:"+border["forwardBorder"],TimeTranslater.getTime(border["forwardBorder"]));
				...
				{//播放点小时 等于最早流所在的小时的情况
					...
					if(playHeadHourMin.min<=forwardHourMin.min){//播放点所在的分钟小于最早流所在的分钟
						Debug.traceMsg(this,"eliminateForback0:"+border["forbackBorder"],TimeTranslater.getTime(border["forbackBorder"]));
						eliminateForback(border);//淘汰最晚方向的一分钟
					}
					else if(playHeadHourMin.min>forwardHourMin.min)
					{
						Debug.traceMsg(this,"eliminateForward1:"+border["forwardBorder"],TimeTranslater.getTime(border["forwardBorder"]));
						eliminateForward(border);//淘汰最早方向的一分钟
					}
				}
				else if(playHeadHourMin.hour>forwardHourMin.hour)
				{//播放点小时 大于最早流所在的小时的情况
					Debug.traceMsg(this,"eliminateForward2:"+border["forwardBorder"],TimeTranslater.getTime(border["forwardBorder"]));
					eliminateForward(border);//淘汰最早方向的一分钟
				}
				Debug.traceMsg(this,"_count = "+_count);
