...
//				"n:"+dataRange.nextConnectBlockID+"_"+dataRange.nextConnectPieceID);
			...
			_count++;
			if(_count>650){
				Debug.traceMsg(this,"_count = "+_count);
			}
			var blockID:Number;
			var block:Block;
			var pieceID:int 
		...
		private function deleteBlockRange(idx:Number):Boolean
		{
			var blockID:Number ;
...
//						_rangeList[block.nextID+"_0"].startBlockID+"_0");
			...
			if(tempBlock)
			{
				for(var i:int=0 ; i<tempBlock.pieces.length ; i++)
				{
					if(tempBlock.pieces[0].stream&&tempBlock.pieces[0].stream.length>0)
					{
						...
						if(i == 0)
						{	
							Statistic.getInstance().removeData(tempBlock.id+"_"+i+"  _count="+_count+""+Debug.getTime(tempBlock.id));
						}else{
		...
		private function eliminate():void
		{
			...
			if(_count>=Config.MEMORY_SIZE/Config.CLIP_INTERVAL)
			{
				Debug.traceMsg(this,"_count = "+_count);
				Debug.traceMsg(this,"_playHead = "+_playHead+" "+Debug.getTime(_playHead));
				...
				var playHeadHourMin:Object=_blockList.getHourMinObj(_playHead);
				var forwardHourMin:Object=_blockList.getHourMinObj(border["forwardBorder"]);
				//判断播放点小时 是否小于最早流所在的小时
				if(playHeadHourMin.hour<forwardHourMin.hour)
				{//播放点所在的小时小于最早流所在的小时
					Debug.traceMsg(this,"eliminateForback0:"+border["forwardBorder"],Debug.getTime(border["forwardBorder"]));
				...
				{//播放点小时 等于最早流所在的小时的情况
					...
					if(playHeadHourMin.min<=forwardHourMin.min){//播放点所在的分钟小于最早流所在的分钟
						Debug.traceMsg(this,"eliminateForback0:"+border["forbackBorder"],Debug.getTime(border["forbackBorder"]));
						eliminateForback(border);//淘汰最晚方向的一分钟
					}
					else if(playHeadHourMin.min>forwardHourMin.min)
					{
						Debug.traceMsg(this,"eliminateForward1:"+border["forwardBorder"],Debug.getTime(border["forwardBorder"]));
						eliminateForward(border);//淘汰最早方向的一分钟
					}
				}
				else if(playHeadHourMin.hour>forwardHourMin.hour)
				{//播放点小时 大于最早流所在的小时的情况
					Debug.traceMsg(this,"eliminateForward2:"+border["forwardBorder"],Debug.getTime(border["forwardBorder"]));
					eliminateForward(border);//淘汰最早方向的一分钟
				}
				Debug.traceMsg(this,"_count = "+_count);
				...
				if(_count>640){
					Debug.traceMsg(this,"_count = "+_count);
					repaireEliminate();
					Debug.traceMsg(this,"淘汰后2:",_toString());
				}
			}
		}
		...
		private function repaireEliminate():void{
			...
			for each(var tempDataRange:DataRange in _rangeList)
			{
				...
				if(_blockList.getHourMinObj(endBlockID).hour<miniHour.hour){
					_rangeList[startBlockID+"_"+startPieceID]=null;
					delete _rangeList[startBlockID+"_"+startPieceID];
				}
				
				if(_blockList.getHourMinObj(endBlockID).hour==miniHour.hour){
					if(_blockList.getHourMinObj(endBlockID).min<miniHour.min){
						_rangeList[startBlockID+"_"+startPieceID]=null;
						delete _rangeList[startBlockID+"_"+startPieceID];
					}					
				}
