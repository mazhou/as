	...
	public class KernelReport
	{
		...
		{			
			...
			if(act != -1)
			{
				if(!obj.utime)
				{
					obj.utime = Math.floor((new Date()).time) - progressReportTime;
				}
				/**上报给播放器,主站目前未贮备该上报*/
				...
				/**上报给内部统计*/
				var str:String = String(stagePath+"act="+act+"&err="+err+"&utime="+obj.utime+"&ip="+obj.ip+"&port="+obj.port+"&gID="+_gID+"&ver="+_ver+"&type="+_type);						
