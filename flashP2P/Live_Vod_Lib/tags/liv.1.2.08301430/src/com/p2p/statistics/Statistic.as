package com.p2p.statistics
{
	...
	public class Statistic
	{
		...
		/**netstream引用*/
		protected var _netStream:P2PNetStream;
		/**心跳周期内http累计下载的字节数*/
		private var _httpsizeNum:Number=0;
		...
		public function set cnodIdx(i:uint):void
		{
			_cnodIdx = i;
		}
		...
		private function streamPlayHandler(evt:EventExtensions):void
		{
			...
			if(outMsg != null)
			{
				outMsg(LiveVodConfig.VERSION,"version");
			}			
			/**设置KernelReport*/
			KernelReport.SET_INFO(LiveVodConfig.VERSION,_initData.groupName,"live")
		...
		/**输出面板调用，显示groupID*/
		private function getGroupID():void
		{
			/**外部输出面板*/
			if(outMsg != null)
			{
			...
			object.info = _initData.groupName;
			testCallBack(object);
		}
		
		/**设置伪直播时间*/
		public function setNetStreamOffTime(livesftime:Number):void
		{
			if( livesftime > 0 
				&& livesftime != LiveVodConfig.TIME_OFF
				&& LiveVodConfig.IS_LIVE_STATE == true)
			{
				LiveVodConfig.TIME_OFF = livesftime;
				_netStream.dispatchStatusEvent({"code":"GSLB.livesftime","time":livesftime});
				_netStream.reseek(LIVE_TIME.GetLiveOffTime());
			}			
		}
		...
		/**输出面板使用*/
		public function isLeader():void
		{
			/**内部输出面板上报*/
			var object:Object = new Object();
			object.name = "time";
			object.info = (LiveVodConfig.IS_REAL_LEAD ? "leader":LiveVodConfig.NAME_OF_LEADER);
			testCallBack(object);
			/**外部输出面板使用*/
		...
		public function rtmfpSuccess(rtmfpName:String,rtmfpPort:uint,myName:String):void
		{
			...
			object.info = LiveVodConfig.VERSION;
			testCallBack(object);
		...
		public function P2PGetData(id:String,begin:Number,end:Number,size:Number,peerID:String):void
		{
			...
			var object:Object = new Object();
			object.id = id;
			if(String(id).split("_")[1]=="0")
			{
				date = new Date(Number(String(id).split("_")[0])*1000);
			...
			if(outMsg != null)
			{
				...
				outMsg(String("--> p2p "+id+" : "+peerID.substr(0,8)));
			}
		...
		public function httpGetData(id:String,begin:Number,end:Number,size:Number):void
		{
			...
			_httpsizeNum += size;
			_downLoadBoo=true;
			P2PDebug.traceMsg(this,"_httpTimeNum = "+_httpTimeNum+"   _httpsizeNum = "+_httpsizeNum)
		...
		private function reportP2PRate():void
		{
			/**内部输出面板使用*/
			var object:Object = new Object();
			object.info = Math.round(Number(1000*(_totalP2PsizeNum + _p2psizeNum)/((_totalHttpsizeNum + _httpsizeNum)+(_totalP2PsizeNum + _p2psizeNum))))/10;
			object.name   = "P2PRate";
			testCallBack(object);
			/**外部输出面板使用*/
			if(outMsg != null)
		...
		/**输出面板使用,节点状态*/
		public function neighborState(obj:Object):void
		{
			/**内部输出面板使用*/
			var object:Object = new Object();
		...
		/**输出面板使用*/
		public function bufferTime(bt:Number,bl:Number):void
		{
			/**外部输出面板使用*/
			if(outMsg != null)
			{				
				outMsg(String(bt+", bufferLength= "+bl),"bufferTime");				
			}
			/**内部输出面板使用*/
			var object:Object = new Object();
			object.info = bl;
		...
		public function setNetStream(netStream:P2PNetStream):void
		{
		...
		public function getStatisticData():Object
		{
			...
			alltimeNum=(_newTime-_perTime)/1000;//秒
			...
			_totalHttpsizeNum += _httpsizeNum;
			_totalHttpTimeNum += _httpTimeNum;
			_totalP2PsizeNum  += _p2psizeNum;
			_totalP2PTimeNum  += p2ptimeNum;
			...
			try
			{
				...
				obj.type     = "live";
				obj.p2ptime  = _totalP2PTimeNum;     //从播放开始到目前p2p下载耗费时间 (毫秒)     Number
				//obj.p2psize  = _totalP2PsizeNum;     //从播放开始到目前p2p下载大小（字节）		Number			
				//obj.httpsize = _totalHttpsizeNum;    //从播放开始到目前http下载大小		        Number
				obj.p2psize  = _p2psizeNum;          //心跳周期内p2p下载大小（字节）		    Number			
				obj.httpsize = _httpsizeNum;         //心跳周期内http下载大小		      		Number
				obj.httpTime = 0;                    //http总下载耗（毫秒）	                    Number???????????????????
				obj.ltime    = _newTime-_perTime;    //心跳周期时长（毫秒）                Number			
				obj.cnod     = _initData.flvNode[_cnodIdx];//当前使用的cdn的标识号
				 ...
				 * type  live或vod
				 * */
				var str:String = String(nativeTrafficPath+"csize="+_httpsizeNum+"&dsize="+_p2psizeNum+"&dnode="+obj.dnode+"&lnode="+obj.lnode+"&gip="+_gIP+"&gport="+_gPort+"&rip="+_rIP+"&rport="+_rPort+"&gID="+_initData.groupName+"&ver="+LiveVodConfig.VERSION+"&type=live");			
				//var str:String = String(nativeTrafficPath+"csize="+obj.csize+"&dsize="+obj.dsize+"&dnode="+obj.dnode+"&lnode="+obj.lnode+"&gip="+obj.gip+"&gport="+obj.gport+"&rip="+obj.rip+"&rport="+obj.rport+"&gID="+_initData.groupName+"&ver="+Config.VERSION+"&type="+"live");			
