...
//			Debug.traceMsg(this,"添加block"+block.id);
		...
		{
			if (this.SeqMap[seq+1])
				return SeqMap[seq+1] as Block;
			else return null;
			//
		...
		public function getHttpTask(seq:Number):Block
		{
			//
			var currentPos:Number = seq;
			while(1)
			{
				if (SeqMap[seq])
				{
					var blk:Block = SeqMap[seq] as Block;
					if (blk._downLoadStat == 0)//空闲
					{
						return blk;
					}
					//
					if (blk._downLoadStat == 2)//p2p正在调度状态
					{
						if (seq - currentPos < 3)//紧急区，必须启动http调度
						{
							return blk;
						}
					}
					//
					if (blk.isChecked == false && blk._downLoadStat == 3)//如果数据出错，即使下载完了，也需要重新下载
					{
						return blk;
					}
					//
		...
		public function getWantPiece(remoteHaveData:Array,farID:String,wantCount:int=3):Array
		{
			...
			for(var j:int=0 ; j<remoteHaveData.length ; j++)
			{
				...
				if(lastBlock&&lastBlock.isChecked == false && lastBlock._downLoadStat != 1)
				{
					var index:int = 0;
					while(1)
					{
		...
		public function getMinuteBlocks(id:Number):Object
		{
			...
			if( _blockList
				&& _blockList[obj.minutes]
				&& _blockList[obj.minutes]["blocks"]
			)
			{
				return _blockList[obj.minutes]["blocks"];
			}
			return null;
		}
		...
		public function eliminate():void
		{
			...
			if(_count>=LiveVodConfig.MEMORY_SIZE/(LiveVodConfig.CLIP_INTERVAL * 5)||_isEliminate)
			{
				_isEliminate=false;
