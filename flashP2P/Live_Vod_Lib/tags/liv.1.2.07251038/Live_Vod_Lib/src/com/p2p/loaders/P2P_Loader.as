...
package com.p2p.loaders
{
	...
	public class P2P_Loader
	{
		...
		/**获得邻居节点中，最早的运行时间*/
		public function getAllPeerMaxHttpLoadPos():Number
		{			
			var earliestStartTimeAllPeer:Number = Number.MAX_VALUE;
			for each(var pipe:P2P_Pipe in _pipeList)
			{
				if (pipe.pipeConnected())
				{
					...
					{
						if (earliestStartTimeAllPeer > pipe.maxHttpPos)
						{
							/**取最小值，值越小，运行时间越早*/
							earliestStartTimeAllPeer = pipe.maxHttpPos;
						}
					}
				}
			}
			//
			return earliestStartTimeAllPeer;
		}
...
//			_initData=evt.data as InitData;
		...
		private function gatherRegisterTimer(event:* = null):void
		{
			 ...
			 * 触发gather心跳条件
				...
				3：geo值是否正确
				4：_pipeSuccessNum>=_MaxConnectedPeers且_isMaxConnectedPeers是否为false
			 * */
			...
			if (_p2pConnection 
				...
				&& _canURLLoader)
			{	
				if(_pipeSuccessNum >= _MaxConnectedPeers  //当成功连接节点数达到最大时
				...
				else if(_pipeSuccessNum >= _MaxConnectedPeers //当成功连接节点数达到最大时
					     && _isFirstRequestAfterMaxPeer)    //并且已经进行最后一次上报
				{
					return;
				}
				else if(_pipeSuccessNum < _MaxConnectedPeers) //当成功连接节点数没有达到最大时
				{
					_isFirstRequestAfterMaxPeer = false;	
				}
		...
		private function peerHartBeatTimer(event:* = null):void
		{
			...
			for(var pipeID:String in _pipeList)
			{
				...
				if(_pipeList[pipeID].pipeConnected())
				{
					...
					//isJoinNetGroup = true;
					/**执行节点心跳*/
					_pipeList[pipeID].peerHartBeatTimer(NearestWantID, this._dispather.getMaxHttpPos());
