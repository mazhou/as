package com.hls_p2p.loaders.p2pLoader
{
	...
	public class SignallingStrategy_V1
	{
		...
		private var _XNetStream:NetStream = null;
		...
		protected var remoteClientType:String 			= "";
		...
		public function set XNetStream(ns:NetStream):void
		{
			if( !_XNetStream )
			{
				_XNetStream = ns;
				_XNetStream.client = this;
				if(!_XNetStream.hasEventListener(NetStatusEvent.NET_STATUS))
				{
					_XNetStream.addEventListener(NetStatusEvent.NET_STATUS, StatusHandler);
				}
				if(!_XNetStream.hasEventListener(AsyncErrorEvent.ASYNC_ERROR))
				{
					_XNetStream.addEventListener(AsyncErrorEvent.ASYNC_ERROR, asyncErrorHandler);
				}
			}			
		}
		...
		{
			console.log(this,"SignallingStrategy_V1 startTransmit called");
			trace(" SignallingStrategy_V1 XNetStream "+event.info.code);
...
//			var version:String = name; 
			if(data)
			{
				...
				if(data.clientType)
				{
					...
					console.log(this,"remoteClientType = "+remoteClientType);
				}
...
//			HITList                 = null;
			...
			/*if (this.XNetStream)
			{			
				if( this.XNetStream === p2p_pipe.XNetStream )
				{
					trace("222 this.XNetStream === p2p_pipe.XNetStream id="+p2p_pipe.remoteID)
				}
				else
				{
					try
					{
						trace("222 this.XNetStream !!! p2p_pipe.XNetStream farID="+this.XNetStream.farID)
			...
			if(_XNetStream)
			{
				...
				if (_XNetStream.hasEventListener(NetStatusEvent.NET_STATUS))
				{	
					_XNetStream.removeEventListener(NetStatusEvent.NET_STATUS,StatusHandler);
				}
				//
				if (_XNetStream.hasEventListener(AsyncErrorEvent.ASYNC_ERROR))
				{
					_XNetStream.removeEventListener(AsyncErrorEvent.ASYNC_ERROR, asyncErrorHandler);
				}
				trace("333 _XNetStream clear() ID = ");
				_XNetStream = null;
			}
