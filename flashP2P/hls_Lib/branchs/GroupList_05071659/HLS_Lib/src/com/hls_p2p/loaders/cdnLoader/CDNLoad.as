package com.hls_p2p.loaders.cdnLoader
{
	...
	public class CDNLoad implements IStreamLoader
	{
		...
		protected var pieceTimeIdx:int 				= 0;
...
//			{
//				console.log(this,"loadByte"+(loadByte-lastLoad)/1024*8/3,LiveVodConfig.DATARATE/5);
...
//		}
		...
		{
			if( LiveVodConfig.cdnDisable == 1 && Statistic.getInstance().connectPeerSuccess == true )
			{
				return;
			}
			if (Task == null)
...
//			}
		...
		{	
			...
			if( Task )
			{
				...
				/*Task.downLoadStat = 0;
				console.log(this,"start Task.downLoadStat = 0"+"bID = "+Task.id);
		...
		public function pause():void
		{
			if(getTaskListTime && true == getTaskListTime.running)
			{
				if(_mediaStream.connected)
				{
					...
					{
						trace("_mediaStream close error")
					}
		...
		private function stopDownloadingTask():void
		{
			...
			//console.log(this,this.id+" set stopDownloadingTask "+Task.id+" "+debugMsg);
...
//				}
			...
			if(range != null)
			{
				...
				console.log(this,this.id+"cdn url:"+url);
				/*if( range != "" )
				{
					Task.downLoadStat = 0;
				}*/
			...
			{
				console.log(this,this.id + "range = null" + this.Task.id + " " + this.Task.name);
				downloadError();
			}
		}
		...
		protected function getDataURL(name:String):String
		{
			var offset:int;
			var fileName:String="";
			console.log(this,"getDataURL",typeof(_initData.flvURL[_initData.g_nM3U8Idx]),_initData.g_nM3U8Idx);
			console.log(this,"getDataURL",_initData.flvURL );
			console.log(this,"getDataURL",_initData.flvURL[_initData.g_nM3U8Idx]);
			if(_initData.flvURL[_initData.g_nM3U8Idx] && 
				(typeof(_initData.flvURL[_initData.g_nM3U8Idx]) == "xml" ||
				typeof(_initData.flvURL[_initData.g_nM3U8Idx]) == "string")
			)//&&  typeof(_initData.flvURL[_initData.g_nM3U8Idx]) == "string"
			{
				var obj:Object = ParseUrl.parseUrl(_initData.flvURL[_initData.g_nM3U8Idx]);
				if(name.indexOf("/")==0)
				{
					return obj.protocol + obj.hostName + name;
				}
				offset = String(obj.path).lastIndexOf("/");
				fileName = obj.protocol + obj.hostName + String(obj.path).substr(0, offset+1);
			}
		...
		protected function getDownloadPieceContentTask(p_arrPieces:Array,pieceId:int=-1):String
		{
			...
			for(var i:int = 0 ; i<Task.pieceIdxArray.length ; i++)
			{
				...
				{							
					...
					{
						debugMsg += "-end:"+i; 
						console.log(this,"-id:" + this.id + debugMsg + " Taskid: " + Task.id);
						//如果是最后一个piece
						if(startByte == 0)
						{
...
//				
//				console.log(this,"id:"+this.id+" Taskid: " + Task.id + " Task.isChecked: " + Task.isChecked + " Task.downLoadStat: " + Task.downLoadStat + " pieceid: " + tmppiece.id + " piece.isChecked: " + tmppiece.isChecked + " piece.iLoadType: " + tmppiece.iLoadType + " piece.isLoad: " + tmppiece.isLoad );
//			}
			
			console.log(this,this.id+" range is null pieceId:"+pieceId + Task.isChecked,Task);
			for(var j:int = 0 ; j<Task.pieceIdxArray.length ; j++)
			{
				piece = dataManager.getPiece(Task.pieceIdxArray[j]);
				console.log(this,j+"piece:",piece);
			}
			return null;
		}
		...
		{
			if(Task && Task.pieceIdxArray && dataManager)
			{
				//错误上报
				var piece:Piece  	= null;
				//var debugMsg:String = "";
...
//			}
		...
		protected function readData_1(isComplement:Boolean=false):void
		{
			...
			catch(err:Error)
			{
				if( Task )
				{
					console.log( this,"bID:"+this.Task.id);
				}
		...
		{
			console.log(this,this.id+"securityErrorHandler: " + event);
			downloadError();
		}
		
		protected function ioErrorHandler(event:IOErrorEvent):void 
		{
			console.log(this,this.id+"ioErrorHandler: " + event);
			downloadError();
		}
		...
		protected function completeHandler(event:Event):void
		{
			...
			if( Task )
			{
				console.log(this,this.id+" completeHandler Taskid: " + Task.id + " name:" + Task.name+" isCheck:" + Task.isChecked );
				stopDownloadingTask();
			}
