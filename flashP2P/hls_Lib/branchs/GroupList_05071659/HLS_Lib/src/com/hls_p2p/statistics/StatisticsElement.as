package com.hls_p2p.statistics
{
	...
	internal class StatisticsElement
	{
		...
		private var _protocol:String = "rtmfp";
		...
		/**心跳周期内来自TV端的p2p累计下载字节数*/
		private var _utsize:Number=0;
		public function get utsize():Number
		{
			return _utsize;
		}
		/**心跳周期内来自手机端的p2p累计下载字节数*/
		private var _umsize:Number=0;
		public function get umsize():Number
		{
			return _umsize;
		}
		/**心跳周期内来自盒子端的p2p累计下载字节数*/
		private var _ubsize:Number=0;
		public function get ubsize():Number
		{
			return _ubsize;
		}
		...
		{
			console.log(this,"clear:"+groupID);
			//最后一次上报
			getStatisticData();
		...
		public function getStatisticData(evt:TimerEvent=null):void
		{
			...
			if( _statistic.connectPeerSuccess == false && dnode == 0 )
			{
				dnode = -2;
			}		
			...
			var gdur:Number  = LiveVodConfig.TYPE!=LiveVodConfig.LIVE ? LiveVodConfig.TOTAL_DURATION : 0;
...
//			}
			...
			var str:String = nativeTrafficPath+"?";
			str += String("csize="+_csize+"&dsize="+_dsize+
				"&streamid="+LiveVodConfig.STREAMID+
				"&tsize="+_tsize+"&msize="+_msize+"&bsize="+_bsize+
				"&lpsize="+(_utsize+_umsize+_ubsize)+
				"&p="+LiveVodConfig.P2P_KERNEL+
				"&dnode="+dnode+"&lnode="+lnode+"&gip="+_gIP+"&gport="+_gPort+
				"&rip="+_rIP+"&rport="+_rPort+"&gID="+groupID+
				"&ver="+LiveVodConfig.GET_VERSION()/*+testID*/+"&type="+LiveVodConfig.TYPE.toLowerCase()+"&termid="+termid+"&platid="+platid+"&splatid="+splatid+
				"&vtype="+LiveVodConfig.VTYPE+"&gdur="+gdur+"&r="+Math.floor(Math.random()*100000));
//			if( this._protocol == "rtmfp")
//			{
//				str += String("&csize="+_csize+"&dsize="+_dsize+"&tsize="+_tsize+"&msize="+_msize+"&bsize="+_bsize+"&dnode="+dnode+"&lnode="+lnode+"&gip="+_gIP+"&gport="+_gPort+"&rip="+_rIP+"&rport="+_rPort+"&gID="+groupID+"&ver="+LiveVodConfig.GET_VERSION()+testID+"&type="+LiveVodConfig.TYPE.toLowerCase()+"&termid="+termid+"&platid="+platid+"&splatid="+splatid+"&vtype="+LiveVodConfig.VTYPE+"&gdur="+gdur+"&r="+Math.floor(Math.random()*100000));		
//			}else if( this._protocol == "utp")
//			{
//				str = nativeTrafficPath+"p=1";
//				str += String("&csize="+_csize+"&dsize="+_dsize+"&tsize="+_utsize+"&msize="+_umsize+"&bsize="+_ubsize+"&dnode="+dnode+"&lnode="+lnode+"&gip="+_gIP+"&gport="+_gPort+"&rip="+_rIP+"&rport="+_rPort+"&gID="+groupID+"&ver="+LiveVodConfig.GET_VERSION()+testID+"&type="+LiveVodConfig.TYPE.toLowerCase()+"&termid="+termid+"&platid="+platid+"&splatid="+splatid+"&vtype="+LiveVodConfig.VTYPE+"&gdur="+gdur+"&r="+Math.floor(Math.random()*100000));
//			}
			console.log(this,"heart:"+str);
		...
		public function P2PGetData(id:String,begin:Number,end:Number,size:Number,peerID:String,clientType:String="PC",protocol:String="rtmfp"):void
		{
