	...
	public class ParseM3U8_uniform
	{
		...
		public function parseFile(file:String,_initData:InitData,p_fun:Function):void
		{
			...
			for( i=0; i < tsList.length; i++ )
			{
				...
				if( 0 != i )
				{
					for( j=0; j<lines.length; j++ )
					{
						...
						if( _tmpFileItem && _tmpFileItem.hasOwnProperty("key") )
						{
							if( _tmpFileItem["key"] != "" )
							{
								if( LiveVodConfig.TYPE != LiveVodConfig.LIVE/*LiveVodConfig.TYPE == LiveVodConfig.VOD*/ && this.hasOwnProperty(_tmpFileItem["key"]) )
								{
				...
				else if( 0 == i )
				{
					for( j=0; j<lines.length; j++ )
					{
						...
						if( _tmpFileItem && _tmpFileItem.hasOwnProperty("key") )
						{
							if( _tmpFileItem["key"] != "" )
							{
								if( LiveVodConfig.TYPE != LiveVodConfig.LIVE && this.hasOwnProperty(_tmpFileItem["key"]) )
								{
									this[_tmpFileItem["key"]]=_tmpFileItem["value"];
								}
								else if( LiveVodConfig.TYPE == LiveVodConfig.LIVE 
				...
				//_clip.groupID =  LiveVodConfig.currentVid
				if(  ( LiveVodConfig.TYPE ==  LiveVodConfig.CONTINUITY_VOD && false == _initData.g_bVodLoaded ) )
				{
					_clip.groupID =  LiveVodConfig.currentVid = this.groupID + EXT_LETV_M3U8_VER + LiveVodConfig.GET_AGREEMENT_VERSION();
					//ttt
					//_clip.groupID =  LiveVodConfig.currentVid = this.groupID;
				}
				else if( LiveVodConfig.TYPE ==  LiveVodConfig.CONTINUITY_VOD && false == _initData.g_bNextVodLoaded )
				{
					//联播加载下一集的m3u8文件
					_clip.groupID =  LiveVodConfig.nextVid = this.groupID + EXT_LETV_M3U8_VER + LiveVodConfig.GET_AGREEMENT_VERSION();
					//ttt
					//_clip.groupID =  LiveVodConfig.nextVid = this.groupID;
				}else
				{
					_clip.groupID =  this.groupID + EXT_LETV_M3U8_VER + LiveVodConfig.GET_AGREEMENT_VERSION();
					//ttt
					//_clip.groupID =  this.groupID;
				}
				//_clip.groupID = "test0";
				//"99991";
				...
				_clip.mediaDuration = this.mediaDuration;
				...
				if( _clip.name!="" )
				{
					...
					{
						console.log( this,"_clip.timestamp is 0" + _clip.url_ts );
					}
			...
			//原来视频的时长
			_initData.mediaDuration = this.mediaDuration;
			...
			{
				kbps = Math.round( (this.fileTotalSize*8/this.totalDuration)/1024 );
			}
			p_fun(_clipList,kbps);
		}
...
//					}
				...
				case "#EXT-LETV-TOTAL-MEDIADURATION:":
					value = getValue(str,"#EXT-LETV-TOTAL-MEDIADURATION:")
					if(value != "")
					{
