package com.hls_p2p.statistics
{
	...
	public class Statistic
	{
		...
		private var _P2PDownloadPieceNum:Number = 0;
		private var _CDNDownloadPieceNum:Number = 0;
		private var _P2PSharePieceNum:Number 	= 0;
		
		/**是否已经与其他节点成功建立过连接*/
		public var connectPeerSuccess:Boolean = false;
...
//			outMsg=null;
			...
			startRunningTimeForDownLoad = 0;
			connectPeerSuccess = false;
			
			_P2PDownloadPieceNum = 0;
			_CDNDownloadPieceNum = 0;
			_P2PSharePieceNum	 = 0;	
		}
		public var startRunningTimeForDownLoad:Number = 0;
		public function startRunningForDownLoad():void
		{
			if( startRunningTimeForDownLoad == 0 )
			{
				startRunningTimeForDownLoad = getTime();
			}
		}
		...
		public function callPlayerDuration(totalDuration:Number):void
		{
			if(outMsg != null)
			{
				outMsg(totalDuration,"totalDuration");
			}
		}
		public function P2PRepeatLoad(id:String,from:String):void
		{
		...
		public function P2PTimeOut(id:String,from:String):void
		{
			...
			object.pieceID    = id;	
			object.from   	  = from;
			object.code       = "P2P.TimeOut.Success";
			(_netStream as EventDispatcher).dispatchEvent(new EventExtensions(NETSTREAM_PROTOCOL.P2P_STATUS,object));
		}
		public function creatStatisticByGroupID(groupID:String):void
		{
			...
			{
				_statisticsElementList[groupID] = new StatisticsElement(this,groupID);
		...
		{
			console.log(this,"delStatisticByGroupID "+id);
		...
		{
			console.log(this,"统计响应play事件"+LiveVodConfig.GET_VERSION()+LiveVodConfig.GET_TEMP_VERSION());
			_initData=evt.data["initData"] as InitData;	
			if(outMsg != null)
			{
				outMsg(LiveVodConfig.GET_VERSION()+LiveVodConfig.GET_TEMP_VERSION(),"version");
			}
		}		
		/**输出面板调用，显示groupID*/
		public function setGroupID(str:String):void
		{
			/**外部输出面板*/
			if(outMsg != null)
			{
				outMsg(str,"groupName");
			}
			/**内部输出面板*/
			var object:Object = new Object();
		...
		/**输出面板显示*/
		public function NEAREST_WANT_ID():void
		{
			/**内部输出面板上报*/
			var object:Object = new Object();
			object.name = "NearWantID";
			object.info = LiveVodConfig.NEAREST_WANT_ID;		
			testCallBack(object);
		}
		/***/
		...
		/**回调播放器，可以加载下一集的视频信息*/
		public function noticeLoadNextData():void
		{
			if(outMsg != null)
			{
				outMsg("","loadNextData");
		...
		public function rtmfpSuccess(rtmfpName:String,rtmfpPort:uint,myName:String,groupID:String):void
		{
			...
			if(outMsg != null)
			{
				...
				tempMyName = String(myName).substr(0,10);
				outMsg(tempMyName,"myName");
			}
			/**内部输出面板*/
			var object:Object = new Object();
			...
			object.info = LiveVodConfig.GET_VERSION()+LiveVodConfig.GET_TEMP_VERSION();
			testCallBack(object);
		...
		/**直播统计下载速度使用*/
		private var tempSpeedObj:Object = new Object();
		private var _downLoadSpeed:Number = 0;
		...
		public function getDownloadSpeed():void
		{
			if( _newTimeForSpeed != 0 )
			{
				...
				if( (_newTimeForSpeed-_oldTimeForSpeed) < 1*1000 )
				{
					return;
				}
			}
			...
			var httpTotalSpeed:Number = 0;
			var p2pTotalSpeed:Number  = 0;
			
			_downLoadSpeed = 0;
			
			if( tempSpeedObj.httpSpeed )
			{
				tempSpeedObj.httpSpeed = 0;
			}
			if( tempSpeedObj.p2pSpeed )
			{
				tempSpeedObj.p2pSpeed = 0;
			}
			for(var i:String in _statisticsElementList)
			{
				tempSpeedObj = (_statisticsElementList[i] as StatisticsElement).dealDownloadSpeed();
				httpTotalSpeed += !tempSpeedObj.httpSpeed ? 0 : tempSpeedObj.httpSpeed;
				p2pTotalSpeed  += !tempSpeedObj.p2pSpeed ? 0 : tempSpeedObj.p2pSpeed;
			}
			
			_downLoadSpeed = httpTotalSpeed+p2pTotalSpeed;
			
			/**内部输出面板使用*/
			var object:Object = new Object();
			object.info = p2pTotalSpeed;
			object.name   = "P2PSpeed";
			testCallBack(object);
			
			var object1:Object = new Object();
			object1.info = httpTotalSpeed;
			object1.name   = "avgSpeed";
			testCallBack(object1);
			
			if(outMsg != null)
			{
				outMsg(tempMyName+" ,HS:"+httpTotalSpeed+" ,PS:"+p2pTotalSpeed,"myName");
			}
			
		}
		
		public function getDownloadSpeed0():void
		{
			if( _newTimeForSpeed != 0 )
			{
				_newTimeForSpeed = getTime();
		 ...
		 * */
		public function P2PGetData(id:String,begin:Number,end:Number,size:Number,peerID:String,groupID:String,clientType:String="PC",protocol:String="rtmfp"):void
		{			
			_tempStatisticsElement = _statisticsElementList[groupID];
			if( _tempStatisticsElement )
			{
				_tempStatisticsElement.P2PGetData(id,begin,end,size,peerID,clientType,protocol);
			}
			//-------------------------------------------------------------------------------------------
			if (null == _netStream) return;
			...
			if(outMsg != null)
			{
				if(protocol == "utp")
				{
					outMsg(String("utp "+object.id+", "+clientType));
				}else
				{
		...
		{	
			...
			if(outMsg != null)
			{
				outMsg(String(object.info+"%, CLoad: "+_CDNDownloadPieceNum+"PLoad: "+_P2PDownloadPieceNum+"Share: "+_P2PSharePieceNum),"p2p下载率");				
			}
		}
		/**输出面板使用,当CDN加载失败时调用*/
		...
		/**输出面板使用,*/
		public function setPeerState(remoteID:String):void
		{
			/**内部输出面板使用*/
			var object:Object = new Object();
			object.name = "peerState";
			var obj:Object = new Object();
			obj.farID   = remoteID;
...
//			console.log(this,"setNetStream");
		...
		{
			if( false == EventWithData.getInstance().hasEventListener(NETSTREAM_PROTOCOL.PLAY) )
			{
