...
//		private var _maxQPeers:uint  = 0;
		...
		public function WebSocket_Loader(_dataManager:DataManager,p2pCluster:P2P_Cluster,selector:Selector)
		{
			this._dataManager 	= _dataManager;
			this.p2pCluster 	= p2pCluster;
			this._selector = selector
			this._selector.addEventListener(Event.COMPLETE,overHandler);
		}
		
		public function startLoadP2P(_initData:InitData,groupID:String,resourceID:String):void
		{
			trace(this+"startLoadP2P");
		...
		{
			_gatherName = this._selector.gatherName;
			_gatherPort = this._selector.gatherPort;
			_hbInterval = this._selector.hbInterval;
		}
		
		public function gatherRegisterTimer(event:* = null):void
		{
			trace("gatherRegisterTimer");
			...
			{					
				...
				{
					var _url:String = "";
					_url += "http://"+_gatherName+":"+_gatherPort+"/cde?termid=1"
					...
					_url += "&pid=33-"+LiveVodConfig.uuid+"-0-0";
					...
					trace(this,"webSocket peerlist:"+_url);
				}
			}
		}
		...
		private function pipeDeadHandler(remoteID:String,idx:int):void
		{
			...
			(_pipeListArr[idx] as SignallingStratey_WS).clear();
		...
		{		
			...
			{	
				...
				if(isDebug)
				{
					obj["peerlist"]=[{userip:'10.58.101.114',pport:'1443',termid:'2','peerid':'1234567'},{userip:'10.58.101.33',pport:'1443',termid:'2','peerid':'1213123123'}];//''10.58.101.33
				}
				...
				for( var i:int = 0 ; i<arr.length ; i++ )
				{
					...
					)
					{
						if( _pipeListArr.length < LiveVodConfig.MAX_PEERS )
						{
							...
							newPipe = new WS_Pipe(groupID,remoteID,arr[i]["userip"],int(arr[i]["pport"]),arr[i]["termid"]);
							_pipeListArr.push(new SignallingStratey_WS(newPipe, this, _dataManager ));
							newPipe.init();
						}
						else
						{
		...
		{
			trace(this,"load UTP Peerlist error:"+evt);
			_canURLLoader = true;
		}
		...
		public function getSuccessPeerList( peerID:String ):Array
		{
			...
			{				
				if(  (_pipeListArr[i] as SignallingStratey_WS).remoteID != ""
					&& (_pipeListArr[i] as SignallingStratey_WS).remoteID != peerID
					&& (_pipeListArr[i] as SignallingStratey_WS).isActivePeer()
					&& true == (_pipeListArr[i] as SignallingStratey_WS).isReceivedData )
				{
					arr.push((_pipeListArr[i] as SignallingStratey_WS).romoteUir);
				}
			}
			return arr;
		}
		...
		{			
			...
			{
				var pipe:SignallingStratey_WS = _pipeListArr[idx];
		...
		private function pushSparePeerIntoPipeList( peerStateObj:Object ):void
		{
			for(var i:int=_sparePipeArr.length-1 ; i>=0 ; i--)
			{
				if( _pipeListArr.length < LiveVodConfig.MAX_PEERS )
				{
					var newPipe:WS_Pipe = new WS_Pipe(groupID,_sparePipeArr[i]['remoteID'],_sparePipeArr[i]["userip"],int(_sparePipeArr[i]["pport"]),_sparePipeArr[i]["termid"]);
					_pipeListArr.push(new SignallingStratey_WS(newPipe, this, _dataManager ));
		...
		private function clearPipeInArr():void
		{
			if( _pipeListArr )
			{
				for( var i:int=_pipeListArr.length-1 ; i>=0 ; i--)
				{
					try
					{
						(_pipeListArr[i] as SignallingStratey_WS).clear();
					}
					catch(err:Error)
					{
		...
		public function clear():void
		{
			...
			if( this._selector )
			{
				this._selector.removeEventListener(Event.COMPLETE,overHandler);
