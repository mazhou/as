package com.hls_p2p.data
{
	...
	public class BlockList 
	{
		...
		/**在点播模式下，根据内存数据大小和码率换算本视频最多保存的播放时长,秒**/
		public var memoryTime:Number = 0;
		...
		public function addGSize(value:Number):void
		{
			_groupList.streamSize += value;
		}
		public function reduceGSize(value:Number):void
		{
			_groupList.streamSize -= value;
		}
		
		public function BlockList( dataMgr:DataManager,groupList:GroupList,kbps:Number=0 )
		{
			dataMgr_ = dataMgr;
			...
			{
				this._kbps = kbps;
				memoryTime = Math.floor( (LiveVodConfig.MEMORY_SIZE*8/1024/kbps) )-(5*60);
			}
		}
		...
		public function addBlock(clip:Clip):Boolean
		{
			...
			if(null != tmpBlock)
			{
				...
				{
					if(tmpBlock.getPiece(0))
					{
						//直播会出现block存在但piece不存在的现象，所以只哟当piece存在的情况下才确定该block空盒子已经下载过！！
						return true;
					}
				}				
			}
		...
		public function deleteCDNIsLoadPiece( piece:Piece ):void
		{
			...
			{
				if( piece == CDNIsLoadPieceArr[i]["piece"] )
				{
			...
			if (block.isChecked == false)
			{
				if( LiveVodConfig.TYPE == LiveVodConfig.CONTINUITY_VOD )
				{
					...
					if(this.groupID == LiveVodConfig.currentVid || 
						this.groupID == LiveVodConfig.nextVid
					)
					{
						/*if( !LiveVodConfig.TaskCache[this.groupID] )
						{
							LiveVodConfig.TaskCache[this.groupID] = new Array();
						}
						LiveVodConfig.TaskCache[this.groupID].push(block.id);
						LiveVodConfig.TaskCache[this.groupID].sort(Array.NUMERIC);*/
						setTaskCache(this.groupID,block);
					}
				}
				else
				{
					...
					LiveVodConfig.TaskCache[LiveVodConfig.currentVid].push(block.id);
					LiveVodConfig.TaskCache[LiveVodConfig.currentVid].sort(Array.NUMERIC);*/
					setTaskCache(LiveVodConfig.currentVid,block);
				}
			}
		...
		private function setTaskCache(gID:String,block:Block):void
		{
			if( !LiveVodConfig.TaskCache[gID] )
			{
				LiveVodConfig.TaskCache[gID] = new Array();
			}
			if( false == block.isChecked )
			{
				LiveVodConfig.TaskCache[gID].push(block.id);
				LiveVodConfig.TaskCache[gID].sort(Array.NUMERIC);
			}
		}
...
//			return getBlockIdLoop( id,id );
		...
		public function gEliminate():void
		{
			this._groupList.eliminate();
		}
		
		public function eliminate( type:String ):void
		{
			...
			if( LiveVodConfig.TYPE == LiveVodConfig.LIVE && 
				( blockArray.length > 300  || this._groupList.streamSize >= LiveVodConfig.MEMORY_SIZE )
			)
			{
				//check left
				for( j = 0; j < blockArray.length;j++ )
				{
					...
					{
						console.log( this,"左侧数据已经淘汰到边界" + block.id );
						break;
					}
					
					realEliminate(block,false);

					console.log( this,"淘汰后: _streamSize:"+this._groupList.streamSize+"("+int(this._groupList.streamSize/1024/1024)+")",blockArray.length);
					cleanUpPieceList();
					return;
				}
				...
				for( j = blockArray.length -1; j >= 0; j-- )
				{
					...
					if( block.id <= LiveVodConfig.ADD_DATA_TIME+LiveVodConfig.TEMP_LIVE_P2P_MAXRANGE/*LiveVodConfig.M3U8_MAXTIME*/ )
					{
						console.log( this,"右侧数据已经淘汰到边界" + block.id );
						cleanUpPieceList();
						break;
					}
					...
					console.log( this,"淘汰后: _this._groupList.streamSize:"+this._groupList.streamSize+"("+int(this._groupList.streamSize/1024/1024)+")",blockArray.length);
					cleanUpPieceList();
					var firstAbsens:Number = 0;
					//block = this.getBlock(blockArray[0]);
					block = this.getBlock(blockArray[LiveVodConfig.ADD_DATA_TIME]);
					
					if( !block && blockArray.length > 0 ){
						block = this.getBlock(blockArray[0]);
						if(block && block.id > LiveVodConfig.ADD_DATA_TIME)
						{
							LiveVodConfig.M3U8_MAXTIME = LiveVodConfig.ADD_DATA_TIME;
							return;
						}
					}
			...
			else
			{
				...
				)
				{
					for(j = 0; j < blockArray.length;j++)
					{
						...
						if( this._groupList.streamSize >= LiveVodConfig.MEMORY_SIZE )
						{
							continue;
						}
						else
						{
							console.log( this,"淘汰后 set memorySize:"+LiveVodConfig.MEMORY_SIZE+" _streamSize:"+this._groupList.streamSize+"("+int(this._groupList.streamSize/1024/1024)+")");
							cleanUpPieceList();
							return;
						}
					}
					
					if( j == blockArray.length )
					{
						cleanUpPieceList();
						return;
					}
				...
				else if( this.groupID == LiveVodConfig.currentVid )
				{
					...
					}else if( "right" == type )
					{
						...
						for(j = blockArray.length -1; j >= 0;j--)
						{
							...
							{
								console.log( this,"淘汰后: _streamSize:"+this._groupList.streamSize+"("+int(this._groupList.streamSize/1024/1024)+")");
								cleanUpPieceList();
								return;
							}
						}
...
//				return;
//			}
//			if( streamSize >= LiveVodConfig.MEMORY_SIZE+1024*1024 || (blockArray.length > 300 && LiveVodConfig.TYPE == LiveVodConfig.LIVE ) )
//			{
//				var i:int = 0;
//				var j:int = 0;
//				
//				console.log( this,"淘汰前 set memorySize:"+LiveVodConfig.MEMORY_SIZE+" _streamSize:"+streamSize+"("+int(streamSize/1024/1024)+")",blockArray.length,LiveVodConfig.ADD_DATA_TIME,LiveVodConfig.BlockID);
//				var block:Block = null;
...
//						else
//						{
//							console.log( this,"淘汰后 set memorySize:"+LiveVodConfig.MEMORY_SIZE+" _streamSize:"+streamSize+"("+int(streamSize/1024/1024)+")");
//							cleanUpPieceList();
//							return;
//						}
//					}
...
//						else
//						{
//							console.log( this,"淘汰后: _streamSize:"+streamSize+"("+int(streamSize/1024/1024)+")");
//							cleanUpPieceList();
//							return;
//						}
//					}
...
////						else
////						{
//							console.log( this,"淘汰后: _streamSize:"+streamSize+"("+int(streamSize/1024/1024)+")",blockArray.length);
//							cleanUpPieceList();
//							return;
////						}
//					}
...
//		}
		...
		private function cleanUpPieceList():void
		{
			...
			if( 0 == this.streamSize && 
				LiveVodConfig.TYPE != LiveVodConfig.LIVE &&
				this.groupID != LiveVodConfig.currentVid && 
				this.groupID != LiveVodConfig.nextVid
			)
			{
				this._groupList.deleteBlockList( this.groupID );
			}
			...
			else if( !isReset )
			{
				...
				if( _blockList && block )
				{
					...
					if( LiveVodConfig.TYPE == LiveVodConfig.CONTINUITY_VOD )
					{
						...
						{
							index = LiveVodConfig.TaskCache[this.groupID].indexOf(block.id);
							if (-1 != index)
							{
								LiveVodConfig.TaskCache[this.groupID].splice(index, 1);
							}
						}
					}else
					{
						index = LiveVodConfig.TaskCache[LiveVodConfig.currentVid].indexOf(block.id);
						if (-1 != index)
						{
							LiveVodConfig.TaskCache[LiveVodConfig.currentVid].splice(index, 1);
						}
					}
