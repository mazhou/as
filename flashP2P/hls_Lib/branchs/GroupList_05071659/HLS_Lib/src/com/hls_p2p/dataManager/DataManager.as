...
//				_descLoad_1.start(_initData);
		...
		public function getCDNRandomTask():Block
		{
				...
				&& this._groupList.CDNIsLoadPieceArr.length > 0 )
			{
				...
				if( tmpPiece )
				{
					...
					{
						console.log(this,"getCDNRandomTask:" + tmpPiece.pieceKey );
		...
		/**添加任务数据desc*/
		public function  writeClipList(clipList:Vector.<Clip>,kbps:Number=0):void
		{
			...
			if( clipList.length > 0 )
			{
				for( var i:int=0;i<clipList.length-1;i++ )
				{
					...
					if( clipList[i].pieceInfoArray.length == 0 )
					{
						if( clipList[i].timestamp < LiveVodConfig.SEEK_TIME )
						{
							continue;
						}else{
						//
							...
							LiveVodConfig.changeBlockId = clipList[i].timestamp;
							return;
						}
					}
					LiveVodConfig.changeBlockPreId = clipList[i].timestamp;
					
					_groupList.addBlock(clipList[i],kbps);
				...
				if( clipList[i].pieceInfoArray.length == 0 && 
					clipList[i].timestamp >= LiveVodConfig.SEEK_TIME )
				{
					//
					console.log(this,"LiveVodConfig.canChangeM3U8 = true");
					LiveVodConfig.canChangeM3U8 = true;
				...
				if( LiveVodConfig.TYPE == LiveVodConfig.VOD 
					|| (LiveVodConfig.TYPE == LiveVodConfig.CONTINUITY_VOD && false == _initData.g_bVodLoaded ) )
				{					
					LiveVodConfig.LAST_TS_ID = clipList[clipList.length-1].timestamp;
				}
			}
			console.log(this,"writeClip:\n"+debugMsg);
			//DO
...
//			_descLoad_1 = null;
		...
		{   
			if( (LiveVodConfig.M3U8_MAXTIME > LiveVodConfig.ADD_DATA_TIME + 2100 && LiveVodConfig.TYPE == LiveVodConfig.LIVE && false == LiveVodConfig.IS_SEEKING ) 
		...
		{
			return _groupList.getMemoryTimeByGid( gID );
		}
	}
}