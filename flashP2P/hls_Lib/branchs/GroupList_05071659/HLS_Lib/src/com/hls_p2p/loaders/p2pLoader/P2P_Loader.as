...
package com.hls_p2p.loaders.p2pLoader
{
	...
	public class P2P_Loader
	{
		...
		private var _hbInterval:uint = 11;//gathert心跳时长
		...
		private var needRespondCloseEvent:Boolean = true;//次变量用来判断侦听器是否应该响应_p2pConnection触发的事件，当主动执行关闭_p2pConnection。close()时needRespondCloseEvent=false；此时不需要侦听器响应
		...
		public function clear():void
		{
			if( _selectorTimer )
			{
				...
				_selectorTimer = null;
				console.log(this,"_selectorTimer clear");
			}
			...
			if( _p2pConnection )
			{
				try
				{
					needRespondCloseEvent = false;
					_p2pConnection.close();
					console.log(this,"_p2pConnection clear _p2pConnection.close() _p2pConnection.connected="+_p2pConnection.connected);															
				}
				catch(err:Error)
				{
				...
				if(_p2pConnection && _p2pConnection.hasEventListener(NetStatusEvent.NET_STATUS))
				{
					_p2pConnection.removeEventListener(NetStatusEvent.NET_STATUS,p2pStatusHandler);					
				}
				if(_p2pConnection && _p2pConnection.hasEventListener(IOErrorEvent.IO_ERROR))
				{
					_p2pConnection.removeEventListener(IOErrorEvent.IO_ERROR,onError);					
				}			
				console.log(this,"_p2pConnection clear _p2pConnection.hasEventListener(NetStatusEvent.NET_STATUS)="+ _p2pConnection.hasEventListener(NetStatusEvent.NET_STATUS));
				console.log(this,"_p2pConnection clear _p2pConnection.hasEventListener(IOErrorEvent.IO_ERROR)="+ _p2pConnection.hasEventListener(IOErrorEvent.IO_ERROR));
				_p2pConnection = null;
				console.log(this,"_p2pConnection clear _p2pConnection="+_p2pConnection);
				
				needRespondCloseEvent = true;
			}			
			
			if( testRestartTimer )
			{
				testRestartTimer.stop();
				testRestartTimer.removeEventListener(TimerEvent.TIMER,testRestartP2PHandler);
				testRestartTimer = null;
			}			
			
			if( _URLLoader )
			{
				if(_canURLLoader == false)
				{
					try
					{
						_URLLoader.close();
		...
		{			
			...
			if( _selector )
			{
				...
				{	
					...
					console.log(this,"rtmfp  = "+_rtmfpName+":"+_rtmfpPort);
					console.log(this,"gather = "+_gatherName+":"+_gatherPort);
					/**过程上报*/
		...
		private function pipeDeadHandler(remoteID:String,idx:int):void
		{
			...
			console.log(this,"pipeDeadHandler",remoteID," id = "+idx);
		...
		{		
			...
			{				
				...
				try
				{
					...
					{
						if( obj.hasOwnProperty("fetchRate") /*&& LiveVodConfig.IS_TEST_ID == true*/)
						{
		...
		{
			console.log(this,"loader_ERROR",evt);
			_canURLLoader = true;
		...
		private var notAllowP2P:Boolean = true;
		/**
		...
		public function peerHartBeatTimer(event:* = null):void
		{
			...
			if( _startPublishNetStreamTime != 0 &&  ( getTime() - _startPublishNetStreamTime > 3*60*1000-15*1000) && notAllowP2P )
			{
				/*_gatherRegisterTimer.stop();
				LiveVodConfig.P2P_KERNEL = "l";
				this.p2pCluster.changeUTP(_gatherName,_gatherPort);*/
				doChangeUTP();
				return;
			}
			
			var changeUTPPeerNum:int = 0;			
			//
			for(var idx:int = _pipeListArr.length-1; idx >= 0; idx--)
			{
				...
				if( pipe.pipeConnected()/*能收发节点数据*/ )
				{
//					Statistic.getInstance().connectPeerSuccess = true;
					_pipeSuccessNum++;
				...
				else
				{
					if( true == notAllowP2P
						&& _pipeSuccessNum == 0 
						&& false == pipe.pipeConnected()
						&& pipe.beginTime > 0
						&& getTime()-pipe.beginTime >= 1*60*1000 )
					{
						changeUTPPeerNum++;
						if( changeUTPPeerNum >= 5 )
						{
							doChangeUTP();
							return;
						}
		...
		private function doChangeUTP():void
		{
			_gatherRegisterTimer.stop();
			LiveVodConfig.P2P_KERNEL = "l";
			this.p2pCluster.changeUTP(_gatherName,_gatherPort);
		...
		private var testRestartTimer:Timer;
		private function testRestartP2P():void
		{
			if(!testRestartTimer)
			{
				testRestartTimer = new Timer(0.5*60*1000,1);
				testRestartTimer.addEventListener(TimerEvent.TIMER,testRestartP2PHandler);
				testRestartTimer.start();
				console.log(this,"______________________testRestartP2PHandler");
			}
			else
			{
				testRestartTimer.reset();
				testRestartTimer.start();
			}			
		}
		private function testRestartP2PHandler(evt:TimerEvent):void
		{
			if( _selectorTimer )
			{
		...
		private function p2pStatusHandler(e:NetStatusEvent):void
		{
			...
			console.log(this,"e.info.code:"+e.info.code);
			switch (e.info.code)
			{
				...
				//case "NetConnection.Connect.NetworkChange" :
					//_publishedOK = false;
					//e.currentTarget
					if( needRespondCloseEvent == true )
					{
						console.log(this,"______________________ "+e.info.code+" ,_p2pConnection.connected"+_p2pConnection.connected);
...
*/
				...
				case "NetStream.Connect.Closed":
					...
					{						
						...
						console.log(this,"______________________ "+e.info.code);
			...
			if(_p2pConnection == null || _p2pConnection.connected == false)
			{
				...
				if( _p2pConnection )
				{
					...
					{		
						needRespondCloseEvent = false;
						_p2pConnection.close();
						console.log(this,"rtmfpTimer _p2pConnection.close() _p2pConnection.connected="+_p2pConnection.connected);
					}
					catch(err:Error)
					{
						console.log(this,"rtmfpTimer error = "+err+" , "+err.getStackTrace());
					}
					console.log(this,"rtmfpTimer _p2pConnection clear _p2pConnection.connected="+_p2pConnection.connected);
				}
				...
				if(!_p2pConnection.hasEventListener(NetStatusEvent.NET_STATUS))
				{
				...
				if(!_p2pConnection.hasEventListener(IOErrorEvent.IO_ERROR))
				{
				...
				if( !_p2pConnection.connected && _rtmfpName != "" && _rtmfpPort != 0 )
				{
					console.log(this,"_p2pConnection.connect  _rtmfpName = "+_rtmfpName+" , _rtmfpPort = "+_rtmfpPort);
					needRespondCloseEvent = true;
					_p2pConnection.connect("rtmfp://"+_rtmfpName+":" + _rtmfpPort +"/");
				}
				
				console.log(this,"_rtmfpName = "+_rtmfpName+" , _rtmfpPort = "+_rtmfpPort);
				/**过程上报*/
			...
			*/
			...
			if( _p2pConnection )
			{
				try
				{
					needRespondCloseEvent = false;
					_p2pConnection.close();
					console.log(this,"onError close() _p2pConnection.connected="+_p2pConnection.connected);							
				}
				catch(err:Error)
				{
				...
				needRespondCloseEvent = true;
				console.log(this,"_p2pConnection.connected = "+_p2pConnection.connected);
			}
		}
			...
			if (_p2pConnection && _p2pConnection.connected)
			{
				if (_publishedOK == false)
				{
					...
					if (_sendNetStream)
					{
						...
						_sendNetStream.removeEventListener(AsyncErrorEvent.ASYNC_ERROR, asyncErrorHandler);
						_sendNetStream = null;
						console.log(this,"_sendNetStream  close  !!!");
						trace("_sendNetStream  close  !!!");
					}
					//
...
//							callerns.client = obj;
							...
							if ( -1 != i )
							{
								...
								if (_pipeListArr[i].canSend == true)
								{
									...
									{
										//callerns.close();//此处与魏帆讨论过，如果执行callerns.close()，会将原来已经建立好的p2p通道也关掉，所以此时只执行return false即可，callerns会被回收
										trace("onPeerConnect return false  farID = "+callerns.farID)
										return false;
									}
									else
									{
										_pipeListArr[i].XNetStream = callerns;
										//在SignallingStrategy_V1中设置client属性
										//_pipeListArr[i].XNetStream.client = _pipeListArr[i].p2p_pipe;
										trace("onPeerConnect return true  farID = "+callerns.farID);
										return true;
									}
							...
							if (_pipeListArr.length >= LiveVodConfig.MAX_PEERS )
							{
								if( _sparePipeArr.length < LiveVodConfig.MAX_PEERS)
								{
