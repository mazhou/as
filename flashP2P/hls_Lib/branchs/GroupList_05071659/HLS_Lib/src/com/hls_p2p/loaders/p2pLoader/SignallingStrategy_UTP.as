...
//			}
		...
		public function SignallingStrategy_UTP(utp_pipe:UTP_Pipe,p2pLoader:UTP_Loader,dataManager:DataManager)
		{
			...
			_peerHartBeatTimer = new Timer(500);
...
//				peerGap = getTime();
		...
		private function sendHartBeat():void
		{
			...
			if( handshakeIsOk &&
				true == LiveVodConfig.ifCanP2PDownload &&
				remotePNList &&
				remotePNList.length>0 &&
				requestArr.length == 0)
			{
				...
				if(piece)
				{
					...
					console.log(this,"sendDataRequest");
				}else{
					console.log(this,"no sendDataRequest");
				}
			}
		...
		public function dataSuccess(receive_byteArray:ByteArray):void
		{
			...
			switch (type)
			{
				...
				case 23:
					...
					break;
				default:
					console.log(this,"default type:"+type);
					break
			}
		}
...
//			}
			console.log(this,"recieve handshake");
			handshakeIsOk = true;
		}
		private function analysisBitField(receive_byteArray:ByteArray):void
		{
			...
			console.log(this,"recieve BitField");
			var size:uint=0;
			switch (type)
			{
				case 0:
...
//					trace("discrete_bitfield");
					console.log(this,"analysisBitField");
					if(receive_byteArray && receive_byteArray.bytesAvailable > 4){
						size = receive_byteArray.readUnsignedInt();
						remotePNList = new Array();
//						console.log(this,"size ="+size);
//						var debugStr:String = "";
						while((receive_byteArray && receive_byteArray.bytesAvailable >= 8 && size>0) )
						{
							size--;
							var pieceIDHigh:uint = receive_byteArray.readUnsignedInt();
							var pieceIDLow:uint = receive_byteArray.readUnsignedInt();
							var pieceID:Number = (pieceIDHigh * 0x100000000) + pieceIDLow;
//							debugStr+=" " +pieceID
							remotePNList.push({"start":pieceID,"end":(pieceID)});
						}
						console.log(this,"remotePNList.length:",remotePNList.length/*,debugStr,remotePNList*/);
					}
					break;
				case 2:
				...
				case 3:
					...
					if(receive_byteArray.bytesAvailable > 4){
						...
						while(receive_byteArray.bytesAvailable >= 10)
						{
							...
							var start:Number = (startHigh * 0x100000000) + startLow;
						...
						console.log(this,"remotePNList.length:",remotePNList.length);
					}
...
//						temp_byteArray.writeUnsignedInt(Math.floor(pieceId & 0xffffffff00000000);
						temp_byteArray.writeUnsignedInt(Math.floor(pieceId/0x100000000));
//						temp_byteArray.writeUnsignedInt(pieceId & 0x00000000ffffffff);
			...
			console.log(this,"send require data:code:22 pieceKey:"+piece.pieceKey);
		...
		public function clear():void
		{
			...
			if(this.p2pLoader)
			{
				this.p2pLoader.clear();
				this.p2pLoader = null;
			}
			
			if(this.utp_pipe)
			{
				utp_pipe.dataSuccess	= null;
				utp_pipe.connectSuccess	= null;
				this.utp_pipe.clear();
				this.utp_pipe = null;
			}
