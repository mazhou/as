package com.hls_p2p.loaders
{
	...
	public class P2P_Pipe extends EventDispatcher
	{
		...
		private var _p2pLoad:P2P_Loader;
		
		private var ifRemoteCanP2P:Boolean = true;
		
		public function get remoteID():String
		{
			return _remoteID;
		}
		public function P2P_Pipe(p2pConn:NetConnection,dispather:IDataManager,gName:String,p2pLoad:P2P_Loader) :void
		{
...
//				bty.uncompress();
			...
			else if( obj.mstype == "removeHave" )
			{
				...
				if(obj.msdata is Array)
				{
					var peerRemoveDataArray:Array = obj.msdata as Array;
					_dispather.removePeerHaveData(_remoteID,peerRemoveDataArray);
				}
			...
			{
				if( obj.msidID>=0/*obj.msidID.hasOwnProperty("NearestWantID") && obj.msidID.NearestWantID != "0"*/ )
				{
					//maxHttpPos = Number(obj.msidID.maxHttpPos);//
					remoteIRHead   = Number(obj.msidID);
					ifRemoteCanP2P = obj.msdata;
					if(ifRemoteCanP2P)
					{
						trace(ifRemoteCanP2P);
					}
					else
					{
						trace(ifRemoteCanP2P);
...
//					this._dispather.SetMaxBlockID(Number(obj.msidID.maxHttpPos));
			...
			if(requestArr)
			{
				...
				while(i>0)
				{
					...
					if(block && block.getPiece(pieceID))
					{
						...
						if(getTime()-block.getPiece(pieceID).begin>5*1000)
						{
							block.getPiece(pieceID).reset(_remoteID);
							requestArr.splice(i,1);
						}
					}
				}
		...
		{
			if( requestArr.length>0 || _p2pLoad.isStartP2P==false )
			{
				return;
			}/**/
		...
		{			
			...
			else if(bl)
			{
				...
				for(var i:int = 0 ; i<obj.pIDArr.length/*bl.pieces.length*/ ; i++)
				{
					...
					if( pieceID < bl.pieces.length
						&& pieceID >= 0
						&& bl.pieces[pieceID].peerHaveData.indexOf(_remoteID) == -1)
					{
		...
		{	
			...
			if (canSend && sendNetStream)
			{
				...
				sendMessage("heartBeat", _NearestWantID/*,_p2pLoad.isStartP2P*/);								
			}	
			//
			checkTimeout();
			sendRequest();
		...
		public function doAddHave():void
		{
			...
			if (_dispather && canSend && sendNetStream && _p2pConnection && _p2pConnection.connected)
			{
				...
				idVector = _dispather.getDataAfterPoint(remoteIRHead);
...
//					sendDataor.checkSum = bl.checkSum;
		...
		//
		public function removeHave(tempEliminateArray:Array):void
		{
			/**
			 * tempEliminateArray数据结构：
			 * tempEliminateArray[i] = {"minuteIdx":Number,"blockIdx":Number,"pieceIdx":Number};
			 * */
			sendMessage("removeHave", _NearestWantID , tempEliminateArray );
		}
		//
