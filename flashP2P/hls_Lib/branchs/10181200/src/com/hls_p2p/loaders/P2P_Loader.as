							...
							{				
								if(arr[i])
								{
									...
									{								
										...
										{
											var newPipe:P2P_Pipe = new P2P_Pipe( _p2pConnection, _dispather, _groupName,this);
...
//			P2PDebug.traceMsg(this,"_pipeSuccessNum ================== "+_pipeSuccessNum);
		...
		public function removeHaveData(tempEliminateArray:Array):void
		{
			...
			for each(var pipe:* in _pipeList)
			{
				...
				{
					pipe.removeHave(tempEliminateArray);
				}
			}
		}
					...
					if(_sendNetStream == null )
					{
						...
						sendStreamClient.onPeerConnect = function(callerns:NetStream):Boolean
						{
							...
							}else
							{
								if( _peerInPipeListNum < LiveVodConfig.MAX_PEERS )
								{
									var newPipe:P2P_Pipe = new P2P_Pipe( _p2pConnection, _dispather, _groupName,myPoint);
		...
		public var isStartP2P:Boolean=true;
