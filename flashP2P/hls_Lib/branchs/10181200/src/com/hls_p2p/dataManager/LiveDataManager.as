package com.hls_p2p.dataManager
{
	...
	import com.hls_p2p.loaders.DescFactory;
...
//		protected var _datLoad2:DATLoader;
		...
		private function init():void
		{
			...
			_descLoad  = new DescFactory().createDescLoader(LiveVodConfig.TYPE,this);
...
//		{
//			return _blockList.getNextblock_BySort(p_LastBufedBlockId);
//		}
		...
		public function getBlockId_withoutseqid(blockId:Number):Number
		{
			return _blockList.getBlockId(blockId);
		}
		...
		{
			return _blockList.getDataAfterPoint_withoutSeqid(id);
		}
		...
		/**添加任务数据desc*/
		public function  writeClipList(clipList:Vector.<Clip>,isMinEnd:Boolean,p_blivevod:Boolean = false):void
		{
			...
			if(clipList.length>0)
			{
				...
				{
					_blockList.addBlock(clipList[i],isMinEnd,p_blivevod);
...
//				P2PDebug.traceMsg(this,"ad：=>"+_initData.getAdRemainingTime());
			...
			if(this.getNearestWantID()-_PlayingBlockID<LiveVodConfig.DAT_BUFFER_TIME/2)
			{
				_p2pLoad.isStartP2P=false;
			}
			else if(this.getNearestWantID()-_PlayingBlockID>LiveVodConfig.DAT_BUFFER_TIME*3/4)
			{
				_p2pLoad.isStartP2P=true;
			}
			//
