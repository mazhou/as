...
//			outMsg=null;
		...
		/**回调onMateData*/
		public function callBackMateData():void
		{
			if (null == _netStream) return;
			//GGG 不在这里调用notify
			...
			if(outMsg != null)
			{
				...
				{
					outMsg(Math.round(_initData.totalSize/(1024*1024))+", W*H="+_initData["videoWidth"]+"*"+_initData["videoHeight"],"totalSize");					
				}
				else
				{
					outMsg("live, W*H="+_initData["videoWidth"]+"*"+_initData["videoHeight"],"totalSize");
				}					
			}
		}
		/**输出面板使用*/
		...
		{	
			...
			if(outMsg != null)
			{
				outMsg(String(object.info+"% ,CDN: "+_CDNDownloadPieceNum+" ,P2P: "+_P2PDownloadPieceNum+" ,Share: "+_P2PSharePieceNum),"p2p下载率");				
			}
		}
		/**输出面板使用,当CDN加载失败时调用*/
