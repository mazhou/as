			...
			{	
				...
				{				
					...
					if( array.length>=4 )
					{
						...
						var tempMaxDelay:int = 2*_hbInterval>60 ? 60 : 2*_hbInterval ;
						if( _maxQPeers>0 && LiveVodConfig.IS_SHARE_PEERS == true )
						{
							...
							if( /*successPeers*/_pipeSuccessNum >= _maxQPeers )
							{
								...
								_gatherRegisterTimer.delay = tempMaxDelay*1000;
								query = 0;
							}
						}
						...
						{
							/**当本地拥有的邻居节点资源大于连接最大值时*/
							/*if( 2*_hbInterval*1000 > 60*1000 )
							{
								_gatherRegisterTimer.delay = 60*1000;
							}
							else
							{
								_gatherRegisterTimer.delay = 2*_hbInterval*1000;
		...
		{		
			...
			{				
				...
				try
				{
					...
					{
//						console.log(this,"fetchRate",obj,obj["fetchRate"]);
//						if( obj["fetchRate"] /*obj["fetchRate"]&& LiveVodConfig.IS_TEST_ID == true*/ )
		...
		{
//			var _browserParams:Object = GetLocationParam.GetBrowseLocationParams();
//			
//			if(_browserParams && ParseUrl.getParam(_browserParams.location,"notAllowP2P") == "true")
//			{
//				this.p2pCluster.changeUTP(_gatherName,_gatherPort);
//				return;
//			}
			if( _startPublishNetStreamTime != 0 &&  ( getTime() - _startPublishNetStreamTime > 3*60*1000) && notAllowP2P )
			{
				_gatherRegisterTimer.stop();
				this.p2pCluster.changeUTP(_gatherName,_gatherPort);
				return;
