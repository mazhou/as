package com.hls_p2p.loaders.descLoader
{
	...
	internal class LiveM3U8Loader implements IDescLoader
	{
		...
		public function LiveM3U8Loader(_dispather:IDispatcher)
		{
		...
		private function handlerDownloadTask(evt:TimerEvent=null):void
		{
			...
			if(timeshift_descTask != -1 || LiveVodConfig.DESC_TIMESHIFT - LiveVodConfig.ADD_DATA_TIME > LiveVodConfig.MEMORY_TIME)
			{
				return;
			}
			_startTime = getTime();
			...
			if (_dispather)
			{
				...
				url = "http://123.126.32.19:1935/hls/ver_00_10_yyyl_test_20s_1024/desc_live.m3u8";
...
//						_initData.videoWidth = int(String(lines[i]).replace("#EXT-LETV-PIC-WIDTH:",""));
				...
				}else if(String(lines[i]).indexOf("#") == -1)
				{
					...
					if(_clip){
						debugStr+="id:"+_clip.timestamp+" drtion:"+_clip.duration+ " seq:"+_clip.sequence+" s:"+_clip.size+" offs:"+_clip.offsize+" block_ck:"+_clip.block_checkSum+" cks:"+_clip.checkSums+" "+_clip.name+"\n"
		...
		{
			timer_timeOut.reset();		
			(new ParseM3U8_uniform).parseFile_1(String(event.target.data),_initData,callbak,this);
			return 0;
		}
