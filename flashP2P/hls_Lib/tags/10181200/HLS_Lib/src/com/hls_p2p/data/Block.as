package com.hls_p2p.data
{
	...
	{	
		...
		public function creatPiece(pieceID:int,size:Number,ifLastPiece:Boolean=false):void{
			...
			{
				pieces[pieceID].isLoad=Math.random()<LiveVodConfig.DAT_LOAD_RATE?true:false;

				if( pieces[pieceID].isLoad )
				{
					P2PDebug.traceMsg(this,"isLoad:"+ this.id + "_" + pieceID);
				}
			}
		...
		private function dispatchGetData(piece:Piece):void
		{
			...
			{
				/**输出面板显示p2p得到数据*/
				Statistic.getInstance().P2PGetData(String(id+"_"+piece.id+"_"+this.sequence),piece.begin,piece.end,Number(piece.getStream().bytesAvailable),piece.peerName);	
			}else
			{
				Statistic.getInstance().P2PGetData(String("_"+id+"_"+piece.id+"_"+this.sequence),piece.begin,piece.end,Number(piece.getStream().bytesAvailable),piece.peerName);
			}
