...
//		private var _headerList:Object = new Object;		
		...
		*/
		...
		public function addBlock(clip:Clip,isMinEnd:Boolean,p_blive:Boolean = false):Boolean
		{
			...
			{
				//P2PDebug.traceMsg(this,"该数据存在："+clip.timestamp);
				/**该block已经存在，不需要添加*/
				return false;
			}
			return realAddBlock(_blockList[_tempDateObj.minutes]["blocks"],clip,p_blive);
		}
		/***设置block属性并添加到list中************/
		private function realAddBlock(blocks:Object, clip:Clip,p_blive:Boolean =false):Boolean
		{
			var block:Block   	= new Block(this);
			block.id           	= clip.timestamp;
			block.checkSums     = clip.checkSums;
			...
			block.checkSum	 	= clip.block_checkSum;
			/**添加block*/
...
//		}
		...
		{	
			if(_blockList&&	_blockList[minute])
			{
				...
				tempEliminateArray.push({"minuteIdx":minute,"blockIdx":-1,"pieceIdx":-1});
