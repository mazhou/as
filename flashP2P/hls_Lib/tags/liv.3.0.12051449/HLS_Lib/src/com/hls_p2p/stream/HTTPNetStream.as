...
//				当用户暂停怎么处理？
				...
				{
					P2PDebug.traceMsg("buffer is empty && delay time>45:"+(LIVE_TIME.GetLiveOffTime()-LiveVodConfig.ADD_DATA_TIME));
...
//				
				...
				{
					P2PDebug.traceMsg("buffer is empty && delay time>90:"+(LIVE_TIME.GetLiveOffTime()-LiveVodConfig.ADD_DATA_TIME));
...
//						
						    ...
						    && (tmpBlock.discontinuity == 0 && tmpBlock.groupID == _currentBlock.groupID) )
						{
							...
							&& tmpBlock.id != _currentBlock.id
							&& ( tmpBlock.discontinuity == 1 || tmpBlock.groupID != _currentBlock.groupID) )
						{
