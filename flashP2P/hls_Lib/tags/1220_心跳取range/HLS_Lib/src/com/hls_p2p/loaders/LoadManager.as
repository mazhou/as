package com.hls_p2p.loaders
{
	...
	public class LoadManager
	{
		...
		public function getP2PTask( getP2PTask:Object ):Object
		{
			...
			for( var i:int = 0; i< LiveVodConfig.TaskCacheArray.length;i++ )
			{
				if( LiveVodConfig.TYPE == LiveVodConfig.VOD && (LiveVodConfig.TaskCacheArray[i] - LiveVodConfig.ADD_DATA_TIME >= LiveVodConfig.MEMORY_TIME) )
				{
					return null;
				}
				...
				if( blk 
					&& ( blk.id - LiveVodConfig.ADD_DATA_TIME <= _CacheLen && false == this._initData.ifP2PFirst() )
					&& (!blk.isChecked) )
				{
				...
				if( blk && ( blk.id - LiveVodConfig.ADD_DATA_TIME > _CacheLen || true == this._initData.ifP2PFirst() ) )
				{
					if( false == blk.isChecked )
					{
