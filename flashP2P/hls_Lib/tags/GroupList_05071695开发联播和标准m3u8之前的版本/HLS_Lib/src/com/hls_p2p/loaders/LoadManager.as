package com.hls_p2p.loaders
{
	...
	public class LoadManager
	{
		...
		public function start( initData:InitData ):void
		{
			...
			var tt:String = LiveVodConfig.THIS_GROUP;
			var tempBlockArr:Array = this.manager.getBlockArray(LiveVodConfig.THIS_GROUP);
			if( !tempBlockArr )
			{
				return;
			}
			...
			for (var i:int = 0; i < tempBlockArr.length; i++)
			{
				if ( tempBlockArr[i] >= LiveVodConfig.ADD_DATA_TIME - 16
					&& manager.getBlock( LiveVodConfig.THIS_GROUP, tempBlockArr[i]).isChecked == false)
				{
					LiveVodConfig.TaskCacheArray.push( tempBlockArr[i] );
				}
			}
			var ttt:Array = LiveVodConfig.TaskCacheArray;
			if( LiveVodConfig.TYPE == LiveVodConfig.CONTINUITY_VOD )
			{
				LiveVodConfig.NextTaskCacheArray = null;
				LiveVodConfig.NextTaskCacheArray = new Array;
				tempBlockArr = this.manager.getBlockArray(LiveVodConfig.NEXT_GROUP);
				
				for (var m:int = 0; m < tempBlockArr.length; m++)
				{
					if ( tempBlockArr[m] >= LiveVodConfig.ADD_DATA_TIME - 16
						&& manager.getBlock( LiveVodConfig.NEXT_GROUP, tempBlockArr[m]).isChecked == false)
					{
						LiveVodConfig.NextTaskCacheArray.push( tempBlockArr[m] );
					}
				}
			}
		...
		protected function handlerGetTaskList():void
		{
			...
			var blk:Block = manager.getBlock( LiveVodConfig.THIS_GROUP,LiveVodConfig.TaskCacheArray[0] );
			if (blk)
			{
				...
				{
					//非直播的紧急区策略
					if( _initData.playlevel == 1 && LiveVodConfig.DURATION>0 )
					{
						disagreeOthersCDNLoaderRun();
		...
		public function getCDNTask( ifLoadAfterBuffer:Boolean ):Object
		{
			...
			{
				P2PDebug.traceMsg(this,"P2PFirst");
				return null;
			}
			//
			if( LiveVodConfig.TaskCacheArray.length == 0)
			{
				P2PDebug.traceMsg(this,"TaskCacheArray.length == 0");
				return null;				
			}
			...
			for( var i:int = 0 ; i < LiveVodConfig.TaskCacheArray.length; i++ )
			{
				...
				//
				var blk:Block = manager.getBlock(LiveVodConfig.THIS_GROUP,LiveVodConfig.TaskCacheArray[i]);
				if( blk && blk.id >= temp )
				{
						...
						if( false == blk.isChecked )
						{
							for(var idx:int = 0;idx<blk.pieceIdxArray.length;idx++)
							{
								...
								if( temPiece && !temPiece.isChecked && temPiece.iLoadType!=1 )
								{
									...
									P2PDebug.traceMsg(this,"getCDNTask:" + blk.id );
		...
		public function getP2PTask( getP2PTask:Object ):Object
		{
			...
			for( var i:int = 0; i< LiveVodConfig.TaskCacheArray.length;i++ )
			{
				...
				var blk:Block = this.manager.getBlock( LiveVodConfig.THIS_GROUP,LiveVodConfig.TaskCacheArray[i] );
...
//					&& (!blk.isChecked) )
//				{
//					P2PDebug.traceMsg(this,"紧急区不p2p");
//					return null;
//				}
				...
				if( blk 
					&& ( blk.id - LiveVodConfig.ADD_DATA_TIME > _CacheLen/*LiveVodConfig.DAT_BUFFER_TIME*/ 
							...
							|| ( LiveVodConfig.cdnDisable == 1 && Statistic.getInstance().userAllowP2P != -1 ) ) )
				{
		...
		public function getM3U8Task():Object
		{
			...
			if( LiveVodConfig.TYPE == LiveVodConfig.LIVE )
			{
				...
				P2PDebug.traceMsg( this,"LiveVodConfig.M3U8_MAXTIME - LiveVodConfig.ADD_DATA_TIME: " + (LiveVodConfig.M3U8_MAXTIME - LiveVodConfig.ADD_DATA_TIME),"timeshift: " + tmpTimeshift );
				...
				{
					//TTT
					P2PDebug.traceMsg(this,"m3u8 set 3000ms");
					tmpDelaytime = 3000;
				}
				else
				{
					//TTT
					P2PDebug.traceMsg(this,"m3u8 set 500ms");
					tmpDelaytime = 500;
				}
