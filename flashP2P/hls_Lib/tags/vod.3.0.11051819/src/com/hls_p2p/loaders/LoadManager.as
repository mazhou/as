package com.hls_p2p.loaders
{
	...
	public class LoadManager
	{
		...
		public function getP2PTask(groupID:String, remoteID:String):Piece
		{
			...
			{
				var temp:Number = this.dispather.getBlockId(LIVE_TIME.GetBaseTime());//
				if(-1 == temp)
				{
					return null;
				}
				if (arr[i] >= temp)
				{
					...
					if ( block	&& block.id - LIVE_TIME.GetBaseTime() > LiveVodConfig.DAT_BUFFER_TIME )
					{							
						if (false == block.isChecked && block.downLoadStat != 1)
						{
							for(j=0;j<block.pieceIdxArray.length;j++)
							{
								...
								if(
									...
									&&
									piece.peerHaveData.indexOf(remoteID)>-1
								)
								{
									piece.peerID = remoteID;
									piece.iLoadType = 2;
									return piece;
								}
							}				
