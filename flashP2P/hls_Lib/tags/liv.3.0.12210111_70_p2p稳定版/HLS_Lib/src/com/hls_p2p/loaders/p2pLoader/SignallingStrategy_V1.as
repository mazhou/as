...
//			var version:String = name; 
		...
		{			
			...
			if (p2p_pipe.canSend /*&& p2p_pipe.canRecieved */&& p2p_pipe.sendNetStream)
			{
				...
				if( true == LiveVodConfig.ifCanP2PUpload )
				{
					...
					data.TNList	= TNList;//this.dataManager.getTNRange(this.groupID);
					data.PNList	= PNList;//this.dataManager.getPNRange(this.groupID);
				}
				else
				{
					data.TNList	= null;
					...
					&& remotePlayType != LiveVodConfig.VOD )
				{
					...
					{
						data.CDNTaskPieceList = HITList;//this.dataManager.getCDNTaskPieceList();
					}
		...
		{
			return ( getTime() - beginTime) > (14 * 1000);
		}
