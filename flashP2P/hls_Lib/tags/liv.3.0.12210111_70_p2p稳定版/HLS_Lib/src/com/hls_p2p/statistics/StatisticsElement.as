package com.hls_p2p.statistics
{
	...
	internal class StatisticsElement
	{
		...
		public function getStatisticData(evt:TimerEvent=null):void
		{
			 ...
			 * r        随机数
			 * */
			var str:String = String(nativeTrafficPath+"csize="+_csize+"&dsize="+_dsize+"&tsize="+_tsize+"&msize="+_msize+"&bsize="+_bsize+"&dnode="+dnode+"&lnode="+lnode+"&gip="+_gIP+"&gport="+_gPort+"&rIP="+_rIP+"&rPort="+_rPort+"&gID="+groupID+"&ver="+LiveVodConfig.GET_VERSION()+"&type="+LiveVodConfig.TYPE.toLowerCase()+"&termid="+LiveVodConfig.TERMID+"&platid="+LiveVodConfig.PLATID+"&splatid="+LiveVodConfig.SPLATID+"&r="+Math.floor(Math.random()*100000));		
