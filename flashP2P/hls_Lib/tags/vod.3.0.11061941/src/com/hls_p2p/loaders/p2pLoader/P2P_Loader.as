	...
	public class P2P_Loader
	{
		...
		public function handlerPiece(piece:Piece):void
		{
			...
			for(i=0;i<piece.peerHaveData.length;i++)
			{
				...
				{
					//如果没有发送数据
					if((_pipeList[piece.peerHaveData[i]] as SigallingStrategy).requestArr.length<=0)
					{
						(_pipeList[piece.peerHaveData[i]] as SigallingStrategy).sendRequest([{
							"type":piece.type,
...
//											newPipe.initPipe(peerID);
											...
											_pipeList[remoteID] = new SigallingStrategy(
												newPipe,
												p2pCluster,
												_dispather
											)
		...
		private function peerHartBeatTimer(event:* = null):void
		{
			...
			for(var remoteID:String in _pipeList)
			{
				...
				//~~~~~~~~~~~~~~~
				peerInfoObj.name  = _pipeList[remoteID].remoteName;//_pipeList[remoteID].remoteID;//
				...
				if(_pipeList[remoteID].canRecieved)
				{
					...
					//isJoinNetGroup = true;
					/**执行节点心跳*/
					_pipeList[remoteID].peerHartBeatTimer(true/*NearestWantID, LiveVodConfig.START_RUN_TIME*/);
