...
package com.hls_p2p.loaders.p2pLoader
{
	...
	public class P2P_Loader
	{
		...
		private var SigallingStrategy:Class = SignallingStrategy_V1;
		...
		public function ifPeerHaveThisPiece( tempPieceObj:Object ):Boolean
		{
			for( var remoteID:String in _pipeList )
			{
				...
				{
					if(_pipeList[remoteID].ifPeerHaveThisPiece( tempPieceObj ))
					{
						tempPieceObj = null;
						return true;
					}
		...
		public function clear():void
		{
			...
			if( _pipeList )
			{
				for ( var str:String in _pipeList )
				{
					try
					{
						(_pipeList[str] as SigallingStrategy).clear();
					}
					catch(err:Error)
					{
								...
								if( arr[i] )
								{
									...
									{								
										if( _p2pConnection && _p2pConnection.connected && _peerInPipeListNum < LiveVodConfig.MAX_PEERS )
										{
											...
											_pipeList[remoteID] = new SigallingStrategy( newPipe, p2pCluster, _dataManager )
		...
		public function peerHartBeatTimer(event:* = null):void
		{
			...
			for(var remoteID:String in _pipeList)
			{
				...
				//~~~~~~~~~~~~~~~
				peerInfoObj.name  = _pipeList[remoteID].remoteName;//
				...
				if(_pipeList[remoteID].canRecieved)
				{
					...
					{/**执行节点心跳*/
						_pipeList[remoteID].PNList = PNRange;
						_pipeList[remoteID].TNList = TNRange;
						_pipeList[remoteID].peerHartBeatTimer(true);
			...
			Statistic.getInstance().getNeighbor(peerStateObj,_pipeSuccessNum,(_peerInPipeListNum+_sparePipeNum),groupID);
			
//			P2PDebug.traceMsg(this,"_pipeSuccessNum ================== "+_pipeSuccessNum);
		}

		private function pushSparePeerIntoPipeList():void
		{
			for(var sparePeer:String in _sparePipeList)
			{
				if( _peerInPipeListNum < LiveVodConfig.MAX_PEERS )
				{
					...
					_pipeList[sparePeer] = new SigallingStrategy(
						newPipe,
						p2pCluster,
						_dataManager
					)
		...
		private function pipeDeadHandler(remoteID:String):void
		{
			/**
			...
			switch (e.info.code)
			{
				...
				case "NetStream.Connect.Success":
					...
					else
					{
						...
						{
							var newPipe:P2P_Pipe = new P2P_Pipe( _p2pConnection,groupID);
							_pipeList[e.info.stream.farID] = new SigallingStrategy(
								newPipe,
								p2pCluster,
								_dataManager
							)
						...
						sendStreamClient.onPeerConnect = function(callerns:NetStream):Boolean
						{
							...
							}else
							{
								if( _peerInPipeListNum < LiveVodConfig.MAX_PEERS )
								{
									...
									_pipeList[callerns.farID] = new SigallingStrategy(
										newPipe,
										p2pCluster,
										_dataManager
									)
