...
package com.hls_p2p.loaders.p2pLoader
{
	...
	public class P2P_Loader
	{
		...
		public function ifPeerHaveThisPiece( tempPieceObj:Object ):Boolean
		{
			for(var i:int=0 ; i<_pipeListArr.length ; i++)
			{
				if(_pipeListArr[i].canRecieved)
				{
					if(_pipeListArr[i].ifPeerHaveThisPiece( tempPieceObj ))
					{
						tempPieceObj = null;
						return true;
					}
		...
		public function peerHartBeatTimer(event:* = null):void
		{
			...
			{				
				if( _pipeListArr[i]["canRecieved"] )
				{
					if ( _pipeListArr[i].isDead() )
					{						
						_badPipeList[_pipeListArr[i]["remoteID"]] = getTime();						
						_pipeListArr[i].clear();
						_pipeListArr[i] = null;
						_pipeListArr.splice(i,1);	
						continue;
					}
					if( _pipeListArr[i]["canRecieved"] && _pipeListArr[i]["canSend"] )
					{
						/**执行节点心跳*/
						_pipeListArr[i].PNList = PNRange;
						_pipeListArr[i].TNList = TNRange;
						
						_pipeListArr[i].peerHartBeatTimer(true);
					}
				}
				if (_pipeListArr[i].isDead())
				{
			...
			_pipeSuccessNum = 0;
			for(var p:int=0 ; p<_pipeListArr.length ; p++)
			{
				var peerInfoObj:Object = new Object();
				peerInfoObj.name  = _pipeListArr[p].remoteName;
				peerInfoObj.farID = _pipeListArr[p].remoteID;
				peerInfoObj.state = "notConnect";	
				
				peerStateObj[_pipeListArr[p]["remoteID"]] = peerInfoObj;
				
				if(_pipeListArr[p].canRecieved && _pipeListArr[p].canSend )
				{
					peerInfoObj.state = "connect";
					_pipeSuccessNum++;
				}
				else if( _pipeListArr[p].canRecieved || _pipeListArr[p].canSend )
				{
		...
		private function p2pStatusHandler(e:NetStatusEvent):void
		{
			switch (e.info.code)
			{
				...
				case "NetStream.Connect.Success":
					...
					{
						_pipeListArr[ifHasPipeInArray(_pipeListArr,e.info.stream.farID)].canRecieved = true;
						...
						sendStreamClient.onPeerConnect = function(callerns:NetStream):Boolean
						{
							...
							}else
							{
								if( _pipeListArr.length < LiveVodConfig.MAX_PEERS )
								{
									...
									/*_pipeList[callerns.farID] = new SignallingStrategy_V1(
										newPipe,
										p2pCluster,
										_dataManager
									)
