...
//			P2PDebug.traceMsg( this,"getNextBlockIdLoop para para_orgID:" + orgID + " para_p_id: " + p_id );
			...
			for each( block in this._blockList )
			{
				...
				if( block )
				{
					if( block.id <= p_id && p_id< Math.round((block.id+block.duration)*100)/100 )
					{
						if( block.id == orgID )
						{
							if( Math.abs(orgID-p_id)>6 )
							{
								P2PDebug.traceMsg( this,"if( block.id == orgID ) not finded block Math.abs(orgID-p_id)>6 orgID:" + orgID + " p_id: " + p_id + "block.id: " + block.id );
								return -1;
							}
			...
			if( Math.abs(orgID-p_id)>6 )
			{
				P2PDebug.traceMsg( this,"not finded block Math.abs(orgID-p_id)>6 orgID:" + orgID + " p_id: " + p_id + "block.id: " + block.id );
				return -1;
			}
		...
		{
			if( _streamSize >= LiveVodConfig.MEMORY_SIZE+1024*1024 || blockArray.length > 30 )
			{
				var i:int = 0;
				var j:int = 0;
				
				P2PDebug.traceMsg( this,"淘汰前 set memorySize:"+LiveVodConfig.MEMORY_SIZE+" _streamSize:"+_streamSize+"("+int(_streamSize/1024/1024)+")",blockArray.length);
				var block:Block = null;
				...
				else if( LiveVodConfig.TYPE == LiveVodConfig.LIVE )
				{
					...
					for( j = 0; j < blockArray.length;j++ )
					{
						...
						// 这里加入淘汰前判断
						if( block.id >= LiveVodConfig.ADD_DATA_TIME-30 )
						{
						...
						if( _streamSize >= LiveVodConfig.MEMORY_SIZE || blockArray.length > 30 )
						{
							continue;
						}
						else
						{
							P2PDebug.traceMsg( this,"淘汰后: _streamSize:"+_streamSize+"("+int(_streamSize/1024/1024)+")",blockArray.length);
							cleanUpPieceList();
							return;
						}
					}
