	...
	internal class ProcessReport
	{
		...
		{			
			...
			if(act != -1)
			{
				...
				/**上报给内部统计*/
				var str:String = String(stagePath+"act="+act+"&err="+err+"&utime="+obj.utime+"&ip="+IP+"&port="+Port+"&gID="+_groupID+"&ver="+LiveVodConfig.GET_VERSION()+"&type="+LiveVodConfig.TYPE.toLowerCase()+"&termid="+LiveVodConfig.TERMID+"&platid="+LiveVodConfig.PLATID+"&splatid="+LiveVodConfig.SPLATID+"&r="+Math.floor(Math.random()*100000));
