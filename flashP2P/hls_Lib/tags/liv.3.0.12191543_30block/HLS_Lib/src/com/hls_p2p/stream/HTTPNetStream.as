...
//			}
			...
			else if( _currentBlock && this.bufferLength > 60 )
			{
				...
				{
					tmpBlock_2 =_dataManager.getNextBlock(_currentBlock.id,LiveVodConfig.ADD_DATA_TIME);
				}
					...
					if( lastPiece != _currentPiece.id )
					{
						...
						else if( (g_Lastbuflen > 0 && this.bufferLength > 0) && (g_Lastbuflen == this.bufferLength) && tmppause == false )
						{
							...
							if( iErrorCount >= 9 )
							{
								...
								{
									tmpBlock_1 =_dataManager.getNextBlock(_currentBlock.id,LiveVodConfig.ADD_DATA_TIME);
								}
