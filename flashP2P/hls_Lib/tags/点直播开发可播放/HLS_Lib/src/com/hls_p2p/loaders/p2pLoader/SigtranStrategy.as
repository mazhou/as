...
//		private var _NearestWantID:Number = 0;
		...
		public function doAddHave():void
		{
			...
			//
			if (dispather && canRecieved && p2p_pipe.sendNetStream && p2p_pipe.p2pConnection && p2p_pipe.p2pConnection.connected)
			{
				/**
				 * idVector元素的数据结构
				 * obj.blockID;
...
//			sendRequest();
		...
		{			
			...
			if(tmpPiece.peerHaveData.indexOf(remoteName) == -1)
			{
				tmpPiece.peerHaveData.push(remoteName);
			}
		}
...
//					}
		...
		{	
			if (p2p_pipe.isConnect && p2p_pipe.sendNetStream)
			{
