package com.hls_p2p.loaders.p2pLoader
{
	...
	import com.hls_p2p.dispatcher.Dispatcher;
	...
	public class P2P_Cluster
	{
		...
		public function initialize(initData:InitData,dispatcher:IDispatcher):void
		{
			if(!isInint)
			{
				p2pList			= new Object;
...
//		}
		...
		public function removeHaveData(simplyPieceList:Array):void
		{
			...
			{
				if(obj[ simplyPieceList[i].groupID ])
				{
					obj[ simplyPieceList[i].groupID ].push( 
						{
							"pieceKey":simplyPieceList[i].pieceKey,
							"type":simplyPieceList[i].type
						} 
					);
				}
				else 
				{
					obj[ simplyPieceList[i].groupID ] = [ {
						"pieceKey":simplyPieceList[i].pieceKey,
						"type":simplyPieceList[i].type
					}  ];	
				}
...
//		}
		...
		protected var dispatcher:IDispatcher;
	}
}