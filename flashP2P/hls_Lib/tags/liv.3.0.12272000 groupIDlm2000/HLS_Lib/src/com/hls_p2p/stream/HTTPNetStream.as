...
//	import flash.external.ExternalInterface;
	...
	public class HTTPNetStream extends NetStream
	{
		...
		private var onLoopDelay:int 				= 0;//25;//100;//
			...
			if( _currentBytes == null || _currentBytes.bytesAvailable == 0 )
			{
					...
					&& _currentPiece.getStream().length>0 )
				{
					...
					if( lastPiece != _currentPiece.id )
					{
						...
						else if( /*(g_Lastbuflen > 0 && this.bufferLength > 0) &&*/ (g_Lastbuflen == this.bufferLength) && ( tmppause == false || _isBufferEmpty == true) )
						{
							++iErrorCount;
							if( iErrorCount >= 8 /*&& this.bufferLength < 0.5*/ )
							{
								iErrorCount = 0;
								...
								if( _currentBlock.nextblkid != -1 )
								{
									...
									//trace("g_Lastbuflen == this.bufferLength = "+this.bufferLength+" ,B= "+_currentBlock.id+" ,P= "+_currentPiece.id+" ,Sure nextB= "+tmpBlock_1.id);
								}
								else
								{
									tmpBlock_1 =_dataManager.getNextBlock( _currentBlock.id );
									//trace("g_Lastbuflen == this.bufferLength = "+this.bufferLength+" ,B= "+_currentBlock.id+" ,P= "+_currentPiece.id+" ,nextB= "+tmpBlock_1.id);
								}
								
								/*notifyMetaData(tmpBlock_1);
								realSeek(tmpBlock_1.id,3);*/
								notifyMetaData(_currentBlock);
								realSeek(_currentBlock.id,3);
								g_Lastbuflen = this.bufferLength;
