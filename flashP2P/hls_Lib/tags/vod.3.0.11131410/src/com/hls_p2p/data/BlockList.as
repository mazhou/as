package com.hls_p2p.data
{
	...
	public class BlockList 
	{
		...
		public  var _blockArray:Array      = new Array;
		...
		private function realAddBlock(clip:Clip):Boolean
		{
			...
			_blockArray.push(block.id);
			_blockArray.sort(Array.NUMERIC);
			return true;
		}
		...
		{
			//return binarySearch(this._blockArray,id);
		...
		public function eliminate():void
		{
			if(_streamSize>=LiveVodConfig.MEMORY_SIZE)
			{
				...
				for(j = 0; j < _blockArray.indexOf(playID);j++)
				{
					if(_blockArray[j]<playID)
					{
						/**淘汰播放点左侧数据*/
						block = this.getBlock(_blockArray[j]);
						if (block.isChecked == false)
							continue;
						//fileMap的block是按照最大的id标识
						realEliminate(block,false,block.id == _blockArray[j]);
				...
				if(_streamSize >= LiveVodConfig.MEMORY_SIZE)
				{
					for(j=_blockArray.length-1;j>=0;j--)
					{
						if(_blockArray[j]>playID+60)
						{
							block = this.getBlock(_blockArray[j]);
							if (block.isChecked == false)
								continue;
							...
							//fileMap的block是按照最大的id标识
							realEliminate(block,true,block.id == _blockArray[j]);
			...
			else
			{
				if(	false == isRight )
				{
					...
					if(isSame)
					{
						...
						_blockArray.splice(_blockArray.indexOf(block.id),1);
