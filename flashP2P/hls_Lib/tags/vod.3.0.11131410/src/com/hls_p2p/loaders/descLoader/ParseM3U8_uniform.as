package com.hls_p2p.loaders.descLoader
{
	...
	public class ParseM3U8_uniform
	{
		...
		public function parseFile(file:String,_initData:InitData,p_fun:Function,descPoint:*=null):void
		{
			...
			for( i=0; i < tsList.length; i++ )
			{
				...
				if( 0 != i )
				{
					for( j=0; j<lines.length; j++ )
					{
						...
						if( _tmpFileItem && _tmpFileItem.hasOwnProperty("key") )
						{
							if( _tmpFileItem["key"] != "" )
							{
								if( LiveVodConfig.TYPE == LiveVodConfig.VOD && descPoint.hasOwnProperty(_tmpFileItem["key"]) )
								{
									descPoint[_tmpFileItem["key"]]=_tmpFileItem["value"];
								}
								else if( LiveVodConfig.TYPE == LiveVodConfig.LIVE && descPoint.hasOwnProperty(_tmpFileItem["key"]) && _tmpFileItem["key"] != "totalDuration" )
								{
									descPoint[_tmpFileItem["key"]]=_tmpFileItem["value"];
								}	
							}
						...
						if( _tmpFileItem && _tmpFileItem.hasOwnProperty("key") )
						{
							if( _tmpFileItem["key"] != "" )
							{
								...
								{
									if( descPoint.hasOwnProperty("DESC_LASTSTARTTIME") )
									{
										descPoint["DESC_LASTSTARTTIME"] = _tmpFileItem["value"];
									}
									
									//P2PDebug.traceMsg(this,"***m3u8_lasttimestamp: " + _tmpFileItem["value"]);
								}
				...
				_clip.groupID = descPoint.groupID + EXT_LETV_M3U8_VER + LiveVodConfig.GET_AGREEMENT_VERSION();
				...
				if( _clip.name!="" )
				{
					P2PDebug.traceMsg(this,"&&&==cliptimestamp: " + _clip.timestamp );
