package com.hls_p2p.statistics
{
	...
	internal class ProcessReport
	{
		...
		public function SET_INFO(ver:String,gID:String,type:String):void
		{
			_ver  = ver;
			_gID  = gID;
			_type = type;
		...
		{			
			...
			if(act != -1)
			{
				...
				/**上报给内部统计*/
				var str:String = String(stagePath+"act="+act+"&err="+err+"&utime="+obj.utime+"&ip="+IP+"&port="+Port+"&gID="+_gID+"&ver="+_ver+"&type="+_type+"&termid="+LiveVodConfig.TERMID+"&platid="+LiveVodConfig.PLATID+"&splatid="+LiveVodConfig.SPLATID+"&r="+Math.floor(Math.random()*100000));
