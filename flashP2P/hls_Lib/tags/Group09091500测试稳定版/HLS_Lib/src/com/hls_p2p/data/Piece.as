...
//				if(true)
				...
				{
					console.log(this,"checkSum有问题！！ "+this.id+",key="+pieceKey+",CS="+checkSum+",CV="+cksValue,this.blockIDArray);
					if(from == "http")
					{
			...
			{
				console.log(this,"size有问题！！ "+this.id+" s="+this.size+" l="+byteArray.length,blockIDArray);
				if(from == "http")
				{
					(errorCount++)>=10?errorCount=10:errorCount;
		...
		public function setStream(byteArray:ByteArray,remoteID:String="",clientType:String="PC"):Boolean
		{
			...
			{			
				...
				console.log(this,"streamPiece:"+this.id+" pKey:"+this.pieceKey+" bID:",this.blockIDArray,this.type,this.size + "_stream.length" + _stream.length );
				
				_blockList.streamSize += size;
				_blockList.addGSize(size);
		...
		private function dispatcherReceiveStream():void
		{
			...
			else
			{
				...
				Statistic.getInstance().P2PGetData(tempStr+blockIDArray+"_"+this.pieceKey,begin,end,size,peerID,groupID,clientType,protocol);
			}
		}
		...
		public function onlyResetParam():void
		{
			...
			if(this._stream.length>0 )
			{
				...
				_blockList.reduceGSize(_stream.length);
				Statistic.getInstance().removeData("reset:"+String(groupID).substr(0,5)+"_"+" bID:"+blockIDArray[0]+"pID:"+this.id+"->"+Math.round(_blockList.streamSize/(1024*1024))+"->"+_blockList.blockArray.length);
			}
		...
		public function reset( blockID:Number ):void
		{
			...
			onlyResetParam();
//			console.log(this,"reset:"+this.id+" pieceKey:"+this.pieceKey+" ty:"+" bID",blockIDArray);
			
		}
		
		private var pieceIndication:Object = null;
		public function getPieceIndication():Object
		{
			if( null == pieceIndication )
			{
				pieceIndication = {"groupID":this.groupID,"pieceKey":this.pieceKey,"type":this.type}
			}
			return pieceIndication;
		}
		public function clear( blockID:Number ):void
		{
			
//			console.log(this,"clearStream: id:"+this.id+" pKey:"+this.pieceKey,"bID:"+blockID+" ,blockIDArray:"+blockIDArray,_blockList);
			if( -1 != blockIDArray.indexOf(blockID) )
			{
				blockIDArray.splice(blockIDArray.indexOf(blockID),1);
				if( blockIDArray.length != 0 )
				{
//					console.log(this,"clearStream: bID:"+blockID+" ,blockIDArray:"+blockIDArray);
					return;
				}
			}
