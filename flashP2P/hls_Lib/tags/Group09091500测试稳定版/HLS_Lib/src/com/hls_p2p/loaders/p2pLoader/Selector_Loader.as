package com.hls_p2p.loaders.p2pLoader
{
	...
	{		
		...
		public var noRequest:Boolean = false;//true;//
		...
		{			
			...
			if(obj["result"] == "success")
			{
				...
				*/
				...
				if(obj["value"].hasOwnProperty("maxPeers"))
				{
					LiveVodConfig.MAX_PEERS = Number(obj["value"]["maxPeers"]);
					//LiveVodConfig.MAX_PEERS = 7;
				}
				
				if(obj["value"].hasOwnProperty("urgentSize"))
				{
					LiveVodConfig.DAT_BUFFER_TIME/*urgentSize*/ = obj["value"]["urgentSize"];
				}
				if(obj["value"].hasOwnProperty("urgentLevel1"))
				{
					LiveVodConfig.DAT_BUFFER_TIME_LEVEL1/*urgentLevel1*/ = obj["value"]["urgentLevel1"];
				}
				
				if(obj["value"].hasOwnProperty("sharePeers"))
				{
					sharePeers = obj["value"]["sharePeers"];
				}
				if(obj["value"].hasOwnProperty("maxQPeers"))
				{
					maxQPeers = obj["value"]["maxQPeers"];
				}
				if(obj["value"].hasOwnProperty("hbInterval"))
				{
					hbInterval = obj["value"]["hbInterval"];
				}
				if(obj["value"].hasOwnProperty("cdnDisable"))
				{
					LiveVodConfig.cdnDisable = int(obj["value"]["cdnDisable"]);
				}
				if(obj["value"].hasOwnProperty("cdnStartTime"))
				{
