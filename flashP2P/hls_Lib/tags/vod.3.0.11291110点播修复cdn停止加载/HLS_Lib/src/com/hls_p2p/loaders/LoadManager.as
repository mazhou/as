package com.hls_p2p.loaders
{
	...
	public class LoadManager
	{
		...
		public function getCDNTask( ifLoadAfterBuffer:Boolean ):Object
		{
			...
			if( TaskCacheNode && TaskCacheNode.task )
			{
				P2PDebug.traceMsg( this,"getCDNTask TaskCacheNode.task.length: " + TaskCacheNode.task.length );
			}
			...
			for( var i:int = 0 ; i<TaskCacheNode.task.length; i++ )
			{
				...
				{
					//TTT
					P2PDebug.traceMsg(this,"getCDNTask temp:Number = this.manager.getBlockId -1 == temp return ");
					return null;
				}
		...
		{
			//TTT
			var tmpblock:Block = manager.getCDNRandomTask();
			if( tmpblock )
			{
				P2PDebug.traceMsg(this,"getCDNRandomTask return blockid:  " + tmpblock.id );	
			}
			else
			{
				P2PDebug.traceMsg(this,"getCDNRandomTask return null:  ");
			}
