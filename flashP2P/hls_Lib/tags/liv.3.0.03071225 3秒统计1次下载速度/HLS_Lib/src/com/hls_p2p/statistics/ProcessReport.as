	...
	internal class ProcessReport
	{
		...
		{			
			...
			if(act != -1)
			{
				...
				var str:String = String(stagePath+"act="+act+"&err="+err+"&utime="+obj.utime+"&ip="+IP+"&port="+Port+"&gID="+_groupID+"&ver="+LiveVodConfig.GET_VERSION()+"&type="+LiveVodConfig.TYPE.toLowerCase()+"&termid="+termid+"&platid="+platid+"&splatid="+splatid+"&r="+Math.floor(Math.random()*100000));
