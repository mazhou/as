package com.hls_p2p.stream
{
	...
	public class HTTPNetStream extends NetStream
	{
		...
		public function HTTPNetStream(p_obj:Object=null)
		{
			...
			console.log(this,"P2PNetStream"+LiveVodConfig.GET_VERSION()+LiveVodConfig.GET_TEMP_VERSION()+" uuid:"+LiveVodConfig.uuid);
//			ExternalInterface.call("version",LiveVodConfig.GET_VERSION());
		...
		private function setBufferTime( isBufferEmptyDispatch:Boolean=false ):void
		{
			...
			console.log(this,"_initData.totalDuration="+_initData.totalDuration);
			if( _initData.totalDuration >= 15 )
			{					
				if( /*getTime()*/this.time-_startPlayTime>=15/*15*1000*/ && _isEmptyIn15==false )
				{
					this.bufferTime = 15;
					console.log(this,"this.bufferTime = 15; "+this.time+"-"+_startPlayTime+"="+(this.time-_startPlayTime));
				}
				else
				{
			...
			else
			{
				...
				{
					this.bufferTime = 2;
					console.log(this,"_initData.totalDuration = "+_initData.totalDuration+"; this.bufferTime = "+this.bufferTime);
				}
				else
				{
					this.bufferTime = 0.1;
			...
			{
				this.bufferTime = 0.1;
				console.log(this,"this.bufferTime = 0.1; "+_initData.totalDuration+"-"+this.time+" = "+(_initData.totalDuration-this.time));
			}
			if( this.bufferTime < 1 )
			{
				LiveVodConfig.BufferTimeLimit = 1;
				console.log(this,"this.bufferTime = "+this.bufferTime+"; LiveVodConfig.BufferTimeLimit = "+LiveVodConfig.BufferTimeLimit);
			}
			else
			{
		...
		override public function play(...args):void 
		{
			...
			{
				console.log(this,"初始化参数"+arg+"=>"+args[0][arg]);
...
//				}
			...
			console.log(this,"gslb securityKey: => "+ _key);*/
...
//			}
		...
		private function notifyMetaData(block:Block = null):void
		{
			...
			if( LiveVodConfig.TYPE == LiveVodConfig.VOD )
			{
				...
				LiveVodConfig.TOTAL_MEDIA_DURATION = metaInfo.mediaDuration = this._initData.mediaDuration;
			...
			if( _currentBytes == null || _currentBytes.bytesAvailable == 0 )
			{
				...
				if( _currentBlock == null )
				{
					...
					if( tmpBlock && _seekType == 0 && -1 != tmpBlock.nextblkid )
					{
						...
						{
							tmpBlock = _dataManager.getBlock( tmpBlock.nextblkid );
						}
					}
					...
					console.log(this,"_startPlayTime = "+_startPlayTime);
					this.bufferTime = 1;
					...
					else
					{
						if( !isPause && _seekDataOK && g_Lastbuflen == this.bufferLength )
						{
							...
							if( iErrorCount >= 8 )
							{
								...
								if( _currentBlock.nextblkid != -1 )
								{
									tmpBlock_3 = _dataManager.getBlock(_currentBlock.nextblkid,true);
								}
								else
								{
									tmpBlock_3 =_dataManager.getNextBlock( _currentBlock.id );
								}
...
//						
								...
								&& tmpBlock.width == _currentBlock.width && tmpBlock.height == _currentBlock.height ) )
						{
							...
							Statistic.getInstance().callBackMateData();
		...
		{
			console.log(this,"isNotifyPlayStop : "+isNotifyPlayStop);
