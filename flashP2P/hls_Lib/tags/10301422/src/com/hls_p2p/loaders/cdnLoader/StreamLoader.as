package com.hls_p2p.loaders.cdnLoader
{
	...
	public class StreamLoader
	{
		...
		public function StreamLoader(_dispather:IDataManager)
		{
...
//			}
		...
		protected function readData(isComplement:Boolean=false):void
		{
			...
			try
			{
				while( _mediaStream.bytesAvailable > 0 )
				{
					_isRead=true;
					pies.clear();
...
//			}
		...
		protected function readData_1(isComplement:Boolean=false):void
		{
			...
			try
			{
				while(_mediaStream.bytesAvailable >= LiveVodConfig.CLIP_INTERVAL)
				{
					...
					tempPiece = Task.getPiece(startDownloadPieceIdx);
					if(tempPiece.isChecked == false && tempPiece.iLoadType != 3)
					{													
						tempPiece.from  = "http";
						tempPiece.begin = startLoadTime+pieceTimeIdx*pieceTime;
						tempPiece.end   = tempPiece.begin+pieceTime;
		...
		protected function handlerDownloadTask(evt:TimerEvent=null):void
		{
			...
			if (_dispather && null == Task)
			{
				...
				var range:String = getDownloadPieceContentTask(Task,m_vecDownloadingPieces,taskObj.pieceId);	
				if(range != null)
				{
		...
		protected function getDownloadPieceContentTask(_task:Block,p_vecPieces:Vector.<Piece>,pieceId:int=-1):String
		{
			return "";
		}
...
//		protected function getDownloadPieceTaskRange_1(_task:Block,pieceId:int=-1):String
//		{	
//			startDownloadPieceIdx = -1;
//			endDownloadPieceIdx   = -1;
//			startLoadTime         = -1;
