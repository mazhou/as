package com.hls_p2p.data.vo
{
	...
	public class Piece
	{
		...
		private function dispatcherReceiveStream():void
		{
			if(from == "http")
			{
				Statistic.getInstance().httpGetData(block.id+"_"+pieceKey+"_"+id,begin,end,size);
			}
			else
			{
				//id:String,begin:Number,end:Number,size:Number,peerID:String,clientType:String="PC"
				Statistic.getInstance().P2PGetData(block.id+"_"+pieceKey+"_"+id,begin,end,size,peerID,clientType);
			}
		}
