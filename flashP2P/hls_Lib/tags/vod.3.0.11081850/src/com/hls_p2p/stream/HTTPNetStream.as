...
//		protected var _flvParser:FLVParser = null;
		...
		private function init():void
		{
			...
			if(!_dispather)
			{
				_dispather=DataManagerFactory.createDispatcher(LiveVodConfig.TYPE);
			}
		...
		{			
			...
			{
				var block:Block = _dispather.getBlock(LiveVodConfig.NEAREST_WANT_ID);
		...
		override public function seek(offset:Number):void
		{
			...
			{
				LiveVodConfig.ADD_DATA_TIME = 1383634971;
			}
...
//				P2PDebug.traceMsg(this,"_currentBytes:"+_currentBytes.bytesAvailable,_currentBytes.length,_currentBlock.id,_currentBlock.size);
		...
		private function fetchData():void
		{
			...
			if(_currentBytes==null || _currentBytes.bytesAvailable==0)
			{
				...
				if(_currentBlock == null)
				{
					...
					//var temp:Number = LIVE_TIME.GetBaseTime();
					//LiveVodConfig.ADD_DATA_TIME = temp;
					tmpBlock=_dispather.getBlock(LiveVodConfig.ADD_DATA_TIME);	
					...
					&& _currentPiece.getStream().length>0)
				{
						...
						&& _currentPiece.id == (_currentBlock.pieceIdxArray.length-1))
					{
						...
						tmpBlock =_dispather.getBlock(LiveVodConfig.ADD_DATA_TIME);//(LIVE_TIME.GetBaseTime() + 5);
		...
		override public function close():void
		{
			...
			super.close();
			Statistic.getInstance().getStatisticData();
			clear();
			//
		}
