package com.hls_p2p.loaders.cdnLoader
{
	...
	public class CDNLoad implements IStreamLoader
	{
		...
		protected function handlerGetTaskList(evt:TimerEvent=null):void
		{
			if (Task == null)
			{
				Task = loadMgr.getCDNTask( this.ifLoadAfterBuffer );
				loadTask();
			}
			else
			{
		...
		{	
			...
			if( Task )
			{
				...
				Task.downLoadStat = 0;
				P2PDebug.traceMsg(this,"start Task.downLoadStat = 0");
		...
		protected function getDownloadPieceContentTask(p_arrPieces:Array,pieceId:int=-1):String
		{
			...
			for(var i:int = 0 ; i<Task.pieceIdxArray.length ; i++)
			{
				...
				if( piece && piece.isChecked == false 
					&&(( piece.iLoadType != 1 && Task.downLoadStat == 1)
					|| (LiveVodConfig.TYPE == LiveVodConfig.LIVE  &&  Task.downLoadStat != 1 && piece.isLoad == true)
					)
				)
				{							
					piece.iLoadType = 1;
			...
			for( var j:int = 0 ; j<Task.pieceIdxArray.length ; j++ )
			{
				...
				P2PDebug.traceMsg(this," Taskid: " + Task.id + " Task.isChecked" + Task.isChecked + " Task.downLoadStat:" + Task.downLoadStat + " pieceid:" + tmppiece.id + " piece.isChecked:" + tmppiece.isChecked + " piece.iLoadType:" + tmppiece.iLoadType + " piece.isLoad:" + tmppiece.isLoad );
			}
		...
		protected function completeHandler(event:Event):void
		{
			...
			{
				Task.downLoadStat = 0;
				P2PDebug.traceMsg(this,"completeHandler Task.downLoadStat = 0");
			}
