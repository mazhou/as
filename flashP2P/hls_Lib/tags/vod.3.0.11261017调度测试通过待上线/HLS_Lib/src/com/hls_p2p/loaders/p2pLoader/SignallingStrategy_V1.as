package com.hls_p2p.loaders.p2pLoader
{
	...
	public class SignallingStrategy_V1
	{
		...
		protected var clientType:String 					= "PC";
...
//				
				...
				{
					this.dispather.clearIsLoaded(data.CDNTaskPieceList);
				}
		...
		public function peerHartBeatTimer(isHeart:Boolean=false):void
		{
			...
			if (p2p_pipe.canSend && p2p_pipe.canRecieved && p2p_pipe.sendNetStream)
			{
				...
				//
				if( remoteBirthTime == -1 || remoteBirthTime > LiveVodConfig.BirthTime )
				{
					data.CDNTaskPieceList = this.dispather.getCDNTaskPieceList();
				}
		...
		protected function outPutP2PState(data:Object):void
		{
			...
			if(data.sendData)
			{
				for(var j:int=0 ; j<(data.sendData as Array).length ; j++)
				{
					try
					{
						tempPiece = dispather.getPiece({"groupID":groupID,"pieceKey":data.sendData[j]["key"],"type":data.sendData[j]["type"]});
						tempPiece.share++;
				...
				if(arr[i])
				{
						...
						&& (arr[i].data as ByteArray).length>0 )
					{
						...
						{	
							...
							tmpPiece.setStream((arr[i].data as ByteArray),p2p_pipe.remoteID,this.clientType);
						}	
