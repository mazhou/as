package com.hls_p2p.data.vo
{
	...
	public class Piece
	{
		...
		{			
			if(from == "http" || LiveVodConfig.TYPE == LiveVodConfig.LIVE)
			{
				return true;
			}
		...
		public function setStream(byteArray:ByteArray):Boolean
		{
			...
			{
				if(LiveVodConfig.TYPE == LiveVodConfig.VOD && byteArray.length != size)
				{
					return false;
				}
				if(LiveVodConfig.TYPE == LiveVodConfig.LIVE && byteArray.length != size && ifLastPiece != true)
				{
					return false;
				}
