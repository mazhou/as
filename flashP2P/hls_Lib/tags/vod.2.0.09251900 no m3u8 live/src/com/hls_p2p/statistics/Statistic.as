...
//			outMsg=null;
		...
		private function streamPlayHandler(evt:EventExtensions):void
		{
			...
			/**设置KernelReport*/
			KernelReport.SET_INFO(LiveVodConfig.VERSION,_initData.groupName,"vod")
...
//			}
		...
		public function P2PGetData(id:String,begin:Number,end:Number,size:Number,peerID:String):void
		{
			...
			/**内部输出面板使用*/
			var object:Object = new Object();
			object.id = id+", "+String(peerID).substr(0,8);
		...
		public function httpGetData(id:String,begin:Number,end:Number,size:Number):void
		{
			...
			_httpsizeNum += size;
			_downLoadBoo=true;
			P2PDebug.traceMsg(this,"_httpTimeNum = "+_httpTimeNum+"   _httpsizeNum = "+_httpsizeNum+" outMsg:"+outMsg);
			...
			if(String(id).split("_")[1]=="0")
			{
				date = new Date(0,0,0,0,0,int(String(id).split("_")[0]));/*new Date(Number(String(id).split("_")[0])*1000);*/
				object.id = object.id+"  "+date.hours+":"+date.minutes+":"+date.seconds;
			}
		...
		public function getStatisticData():Object
		{
			...
			try
			{
				...
				obj.p2ptime  = p2ptimeNum;    		 //心跳周期内p2p下载耗费时间 (毫秒)     Number
				 ...
				 * type  live或vod
				 * */
				var str:String = String(nativeTrafficPath+"csize="+_httpsizeNum+"&dsize="+_p2psizeNum+"&dnode="+obj.dnode+"&lnode="+obj.lnode+"&gip="+_gIP+"&gport="+_gPort+"&rip="+_rIP+"&rport="+_rPort+"&gID="+_initData.groupName+"&ver="+LiveVodConfig.VERSION+"&type=vod");			
