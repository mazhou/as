	...
	public class Statistic
	{
		...
		{			
			...
			if(heartBeatReportTimer)
			{
				...
				heartBeatReportTimer.removeEventListener(TimerEvent.TIMER,heartBeatReportTimerHandler);
			}			
...
//			outMsg=null;
		...
		private function streamPlayHandler(evt:EventExtensions):void
		{
			...
			if( !heartBeatReportTimer )
			{
				heartBeatReportTimer = new Timer(3*60*1000)
				heartBeatReportTimer.addEventListener(TimerEvent.TIMER,heartBeatReportTimerHandler);
		...
		private function heartBeatReportTimerHandler(evt:TimerEvent):void
		{
			getStatisticData();
		}
...
//			}
		...
		public function P2PGetData(id:String,begin:Number,end:Number,size:Number,peerID:String,clientType:String="PC"):void
		{
			...
			if(outMsg != null)
			{
				...
				outMsg(String("p2p "+object.id+" : "+peerID.substr(0,8)+", "+clientType));
			}
		...
		public function httpGetData(id:String,begin:Number,end:Number,size:Number):void
		{
			...
			if( id.indexOf("TN") > -1)
			{
				date =new Date(int(id.split("_")[1])*1000);/*new Date(Number(String(id).split("_")[0])*1000);*/
				object.id = object.id+" "+date.hours+":"+date.minutes+":"+date.seconds;
			}
		...
		/**输出面板使用,当给别人分享数据时调用*/
		public function P2PShareData(pieceID:String,remoteID:String/*,size:Number*/):void
		{
			if (null == _netStream) return;
			/**输出面板使用*/
			var object:Object = new Object();			
