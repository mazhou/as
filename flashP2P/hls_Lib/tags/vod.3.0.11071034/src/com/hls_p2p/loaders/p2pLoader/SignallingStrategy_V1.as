...
//			var version:String = name; 
		...
		protected function outPutP2PState(data:Object):void
		{
			...
			if(data.sendData)
			{
				...
				{
					Statistic.getInstance().P2PShareData(data.sendData[i]["type"]+"_"+data.sendData[i]["key"],p2p_pipe.remoteID);
				}
			}			
		}
					...
					if(arr[i].hasOwnProperty("type"))
					{
						if("TN" == arr[i]["type"])
						{
							...
							)	
							{
								...
								if(
									tmpPiece.checkSum == arr[i]["checksum"]
									&&
									tmpPiece.isChecked
									&&
									tmpPiece.getStream().length>0
								)
								{
									if(null == readySendData)//讨论
									{
										readySendData = 
											{
												"type":	tmpPiece.type,
