...
//			return getBlockIdLoop( id,id );
		...
		public function eliminate():void
		{
			...
			if( _streamSize >= LiveVodConfig.MEMORY_SIZE+1024*1024 || (blockArray.length > 300 && LiveVodConfig.TYPE == LiveVodConfig.LIVE ) )
			{
				...
				P2PDebug.traceMsg( this,"淘汰前 set memorySize:"+LiveVodConfig.MEMORY_SIZE+" _streamSize:"+_streamSize+"("+int(_streamSize/1024/1024)+")",blockArray.length,LiveVodConfig.ADD_DATA_TIME,LiveVodConfig.BlockID);
				var block:Block = null;
...
//						}
					...
					for( j = blockArray.length -1; j >= 0; j-- )
					{
						...
						trace("右侧淘汰 + "+block.id + " ,ADD_DATA_TIME = "+LiveVodConfig.BlockID)
						realEliminate(block,false);
