	...
	public class P2P_Loader
	{
		...
		private function gatherRegisterTimer(event:* = null):void
		{
			...
			if( _p2pConnection 
				...
				&& _canURLLoader )
			{	
				if( _pipeListArr.length >= LiveVodConfig.MAX_PEERS  //当成功连接节点数达到最大时
				...
				else if( _pipeListArr.length >= LiveVodConfig.MAX_PEERS //当成功连接节点数达到最大时
					     && _isFirstRequestAfterMaxPeer )    //并且已经进行最后一次上报
				{
					return;
				}
				else if( _pipeListArr.length < LiveVodConfig.MAX_PEERS ) //当成功连接节点数没有达到最大时
				{
					_isFirstRequestAfterMaxPeer = false;	
				}
		...
		public function peerHartBeatTimer(event:* = null):void
		{
			...
			//
			Statistic.getInstance().getNeighbor(peerStateObj,_pipeSuccessNum,_pipeListArr.length,groupID);
			/*
			trace("#######################################################");
			if(_pipeListArr.length> LiveVodConfig.MAX_PEERS)
			{
...
//							callerns.client = obj;
							...
							if ( -1 != ifHasPipeInArray(_pipeListArr,callerns.farID) )
							{
								...
								{
									_pipeListArr[ifHasPipeInArray(_pipeListArr,callerns.farID)].XNetStream = callerns;
									_pipeListArr[ifHasPipeInArray(_pipeListArr,callerns.farID)].XNetStream.client = _pipeListArr[ifHasPipeInArray(_pipeListArr,callerns.farID)].p2p_pipe
								    return true;
								}
								//
