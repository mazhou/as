package com.hls_p2p.data
{
	...
	{	
		...
		public function liveDataIsOK():Boolean
		{
			var piece:Piece = null;
			for each( var simplyPiece:* in pieceIdxArray )
			{
				piece = this._blockList.getPiece(simplyPiece);
				if( piece.isLoad == true && piece.isChecked == false )
				{
					return false;
				}
			}
		...
		public function reset(isClear:Boolean=false):void
		{
			...
			Statistic.getInstance().removeData(this.id+"->"+Math.round(_blockList.streamSize/(1024*1024)));
		}
	}
}