package com.hls_p2p.statistics
{
	...
	internal class StatisticsElement
	{
		...
		private var heartTime:int = 3*60*1000;//10*1000;//
		...
		public function getStatisticData(evt:TimerEvent=null):void
		{
			 ...
			 * r        随机数
			 * */
			var str:String = String(nativeTrafficPath+"csize="+_csize+"&dsize="+_dsize+"&tsize="+_tsize+"&msize="+_msize+"&bsize="+_bsize+"&dnode="+dnode+"&lnode="+lnode+"&gip="+_gIP+"&gport="+_gPort+"&_rIP="+_rIP+"&_rPort="+_rPort+"&gID="+groupID+"&ver="+LiveVodConfig.GET_VERSION()+"&type="+LiveVodConfig.TYPE+"&termid="+LiveVodConfig.TERMID+"&platid="+LiveVodConfig.PLATID+"&splatid="+LiveVodConfig.SPLATID+"&r="+Math.floor(Math.random()*100000));		
			sendToURL(new URLRequest(str));
			trace(this,String(nativeTrafficPath+"csize="+_csize+"&dsize="+_dsize+"&tsize="+_tsize+"&msize="+_msize+"&bsize="+_bsize+"&dnode="+dnode+"&lnode="+lnode+"&gip="+_gIP+"&gport="+_gPort+"&_rIP="+_rIP+"&_rPort="+_rPort+"&gID="+groupID+"&ver="+LiveVodConfig.GET_VERSION()+"&type="+LiveVodConfig.TYPE+"&termid="+LiveVodConfig.TERMID+"&platid="+LiveVodConfig.PLATID+"&splatid="+LiveVodConfig.SPLATID+"&r="+Math.floor(Math.random()*100000)))
			reset();
